...
النقطة الآمنة المعتمدة هي **v3.48.0** — آخر إصدار بُني ووُقّع ورُكّب وفُتح على
جهاز بلا انهيار، بتاريخ 2026-09-20.

| | |
|---|---|
| الوسم | `v3.48.0` |
| الفرع الثابت | `known-good/v3.48.0` — **لا يُحرّك ولا يُحذف** |
| الوسم الاحتياطي | `backup-2026-09-20` — نفس الكوميت، اسم يسهل تذكّره |
| الـAPK | `dist/RafeeqAlDarb-v3.48.0.apk` وعلى GitHub Releases |
...
git fetch --all --tags
git switch -c rescue-$(date +%Y%m%d) known-good/v3.48.0
```
...
```bash
git switch master
git reset --hard known-good/v3.48.0
...
```bash
adb install -r "E:\My Projects\Rafiq-Al-Darb\dist\RafeeqAlDarb-v3.48.0.apk"
```
...
أو نزّله من:
https://github.com/tito423/Rafeeq-Al-Darb/releases/tag/v3.48.0
...
ثم `grep "^version:" rafeeq_app/pubspec.yaml` ← `3.48.0+50`.
