# AGENTS.md — how work is done on Rafiq Al-Darb

**This file is mandatory.** Codex loads it automatically every session.
Any agent working on this project — Codex, Antigravity, Cline, Gemini,
...
1. **Check the remaining budget at the start of the session**, before promising
   anything. In Codex that is `/usage` (or `/status`) in an interactive
...
**It is switched on when the session STARTS, and only then.**
    ...
    Codex rc            # or: Codex --remote-control [name]

That is `Codex remote-control`, «Control local sessions from Codex.ai/code or
the Codex mobile app», and it **starts a new interactive session** with Remote
...
running `Codex rc` from a tool call only spawns a process the owner is not
sitting in front of. (Checked: `Codex rc --help` prints exactly that usage,
and a bare `Codex rc` blocks waiting for a terminal.)
...
reply and let him decide whether to restart with `Codex rc`. Never claim to
