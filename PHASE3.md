...
## P3-45 — v2.1.5 real-device round: notification crash, locale staleness, downloads nav, orientation, splash removal (2026-09-05)

Nine real bugs from a fresh round of device testing on v2.1.5 (10+ screenshots plus two follow-up messages), each root-caused live rather than patched from the screenshot alone.

**1. Home prayer card showing a generic "something went wrong" error in ES/PT/FR (screenshots).** Root cause found by cold-starting the app and reading `logcat`, not guessed: `flutter_local_notifications` throws `RuntimeException: Missing type parameter` from its own persisted scheduled-notification storage — reproduced two ways: (a) `ScheduledNotificationBootReceiver` on cold start after `BOOT_COMPLETED`/`am force-stop`, a genuine `FATAL EXCEPTION` that kills the whole process (confirmed 100% reproducible, not "once" as first guessed in P3-44); (b) the same exception thrown from `.zonedSchedule()`/`.cancel()` calls made from Dart. `rescheduleAdhans` (`adhan_scheduler.dart`) called this unguarded inside `PrayerController._load()`, so ANY prayer-times load that also tried to (re)arm an adhan alarm took the *entire* Home card down with it — real prayer times hidden behind an unhelpful error that had nothing to do with the times themselves. Fixed by making every plugin call defensive at its own layer, matching the "notifications are optional" rule already applied to `PrayerStatusNotification`:
  - `adhan_scheduler.dart`: each prayer's `scheduleDaily` wrapped per-iteration so one failing alarm never blocks the other four or the times themselves.
  - `prayer_status_notification.dart`: `refresh()`'s whole body (was unguarded) and `hide()` now try/catch.
  - `adhan_alarm_service.dart`: `stopById`/`muteById`/`cancelPrayer`/`cancelAll` now try/catch — `stopById` specifically matters because it's the full-screen alert's own Stop button; before this fix a live adhan's Stop tap could silently fail to cancel the notification/sound while still visually "pressing" the button.
  - **Not fixed (needs its own dedicated look, flagged not guessed)**: the *native* `ScheduledNotificationBootReceiver` crash lives entirely inside the compiled plugin, unreachable from Dart try/catch — the real fix is a `flutter_local_notifications` version bump (pinned at 18.0.1; 22.3.0 available) or a data-migration for whatever legacy-format SharedPreferences entry is failing to deserialize. Plausibly connects to the whole session's "adhan notification killed/disappeared" family of complaints, not just this round's Home-card symptom.

**2. Header "Bienvenue"/"Bem-vindo" wrapping mid-word across 2-3 lines in FR/PT (screenshots).** `home_screen.dart`'s `_HeaderCard` used the Quran-calligraphy `AmiriQuran` font for the welcome text in *every* locale, and Latin glyphs in that font are far wider than its Arabic ones at the same size — squeezed between two date labels (English `DateFormat.yMMMd` output is longer in fr/pt than the Hijri line was designed around), it had nowhere to go but wrap. Fixed: `AmiriQuran` now only applies for `ar`; every other locale uses the app's normal font. Added `maxLines: 1` + `overflow: TextOverflow.ellipsis` to all three header `Text`s (welcome + both date labels) as a hard safety net regardless of locale.

**3. Permissions section (and Sunnah Surahs Reminders section) frozen in a stale locale after switching languages inside Settings (screenshots) — reproduced live, root-caused precisely.** Both were instantiated as `const PermissionsSection()` / `const SunanSuwarRemindersSection()` in `settings_screen.dart`. `.tr()` reads easy_localization's own global current-locale state, not a `BuildContext` InheritedWidget dependency — nothing marks a `.tr()`-only widget dirty on locale change by itself. Flutter's own `Element.updateChild` has an `identical(oldWidget, newWidget)` fast path that skips calling `build()` entirely when a parent keeps re-passing the same canonicalized `const` instance — which a no-argument `const` constructor always is. Dropping `const` at both call sites was the fix (constructors themselves stay `const`-declared; only the *instantiation* changed) — a genuinely new, non-identical widget each rebuild takes the normal update path and calls `build()` again with fresh translations, with zero state loss (same `runtimeType` + no key still reuses the same `Element`/`State`).

This turned out to be a systemic pattern, not a one-off: `AppShell`'s own `IndexedStack` — `Home`/`Quran`/`Prayer`/`Azkar`/`Tasbeeh`/`Library` — had the *exact* same bug for five of its six tabs (`QuranScreen()`, `QiblaScreen()`, `AzkarScreen()`, `TasbeehScreen()`, `LibraryScreen()` were all `const`; only `HomeScreen` wasn't, because its `onNavigate` closure parameter makes a `const` constructor impossible — which is exactly why Home alone translated correctly when tested). Confirmed live: after switching Settings → English, `LibraryScreen`'s own AppBar title and TabBar labels ("Library" / "Hadith" / "Available books") stayed in English even after switching the app back to Arabic, while everything else on screen correctly re-translated. Fixed by dropping `const` from all five in `app_shell.dart`'s `screens` list — `IndexedStack` still keeps each tab's `State` alive exactly as before (that never depended on the `const` keyword), they just actually re-render their own chrome on the rare `AppShell` rebuild (tab switch/locale/theme) instead of being frozen after their first frame.

**4/5. Downloads screen: "Hadith"/"Books" category rows did nothing beyond closing Downloads itself; "Adhan" row was inert ("fake tabs", the owner's own words).** Root cause for Hadith/Books: `DownloadsScreen` is pushed from `SettingsScreen`, itself pushed from `AppShell` — the row's `onTap` did a single `Navigator.pop()` (closing only `DownloadsScreen`, landing back on `Settings`) *then* set the `requestedTabProvider`/`requestedLibraryTabProvider` seam, but with `AppShell` still buried under `Settings` nothing visible ever happened — the exact bug class `tab_request_provider.dart`'s own doc already names from an earlier session ("افتح المصحف silently did nothing but pop back to Home"). Fixed: `popUntil((route) => route.isFirst)` instead of a single `pop()`, so the whole pushed stack clears back to `AppShell` where the tab switch is actually seen. For Adhan (previously `onTap: null`, "no download-browsing UI anywhere in the app yet") — that's no longer true; adhan videos are downloaded from the Adhan settings screen (built earlier this session), so the row now pushes `AdhanSettingsScreen` directly instead of rendering as a tappable-looking dead end.

**6. Rotation/orientation didn't work at all.** `main.dart` hard-locked the whole app to `[portraitUp, portraitDown]` via `SystemChrome.setPreferredOrientations` at startup — a leftover from early development, not something any screen's layout actually required. Removed entirely (no call at all, so the OS's own auto-rotate setting decides, like almost every other app). Live-verified on the emulator: rotated Home and the Quran mushaf reader to landscape — both reflow correctly with ordinary `Column`/`ListView`/`Scaffold` layout, no crash, no clipped content, bottom nav and toolbar all fit. Not a bespoke landscape redesign (the mushaf page just re-wraps across a wider single measure rather than mimicking a two-page-spread), but genuinely functional, which is what was asked.

**7. Splash screen removed outright (explicit instruction, not a bug report).** Deleted `lib/features/splash/` wholesale (`splash_screen.dart`, `splash_video_provider.dart`, `splash_lattice.dart`) — the branded video/lattice-badge intro, its first-run-only/toggle logic, and the "Splash video" Settings switch all gone together, not left half-wired. `rafeeq_app.dart`'s `home:` now reads `onboardingCompletedProvider` (already a synchronous SharedPreferences read) directly and picks `AppShell` or `OnboardingScreen` — no screen, no brand pause, in between. Also deleted the now-fully-unused `assets/branding/` folder (`splash_intro.mp4` + `app_mark.png` + a stray final-frame PNG, ~5MB) and its `pubspec.yaml` bundling line, and the `settings.splash_video`/`splash_video_desc` keys from all 6 locale files. Live-verified end to end on a clean uninstall+reinstall: cold start goes straight from Android's own native launch screen to the notification-permission system dialog to onboarding, zero custom splash frame anywhere in between.

**8. Downloads-hang-when-backgrounded ("any process like download, then put app in background everything and any thing hang and stop") — investigated, root cause identified, NOT fixed this round, flagged rather than band-aided.** `DownloadManager._run()`'s own download loop (`download_manager.dart`) has no bug found in it — no locks, no blocking synchronous calls, every plugin call it touches (`DownloadNotifications.showProgress`) was already try/catch-guarded. The far more likely explanation, matching this session's already-established OEM-background-killing pattern (autostart settings, battery-optimization exemption): the app has no genuine Android foreground service for active downloads, only a plain "ongoing" notification (`ongoing: true` is cosmetic, it doesn't protect the process). Android can freeze or deprioritize a cached (backgrounded) app's whole process — not just its network socket — especially on power-management-aggressive OEMs, which would stall *everything* in this single-isolate app at once (matching "everything hangs," not just the download) until the app is foregrounded again. A real fix needs a genuine Android foreground service tied to active downloads, which is a substantial native feature, not a Dart-side tweak — not attempted blind without the owner's real device to verify against.

flutter analyze clean, flutter test 21/21 throughout. Live-verified on emulator-5554 across a full release rebuild+reinstall cycle: prayer times load with no error card, header renders correctly in en/ar without wrapping, Settings→Arabic correctly retranslates the Permissions/Sunnah-Reminders sections live, Downloads→Books correctly lands on the Library tab, landscape rotation works on Home and the Quran reader, and the splash is completely gone from a fresh install.
