...
void main() {
  ...
  test(
    'a correct recitation misheard by the model still counts (Maryam 3)',
    () {
      final r = TasmeeEngine.compare(
        ayahText: maryam3,
        heard: 'إذن دعوا به ندى أن خفية',
      );
      expect(r.total, 5);
  ...
  test('a word split in two by the model still counts (Maryam 2)', () {
    final r = TasmeeEngine.compare(
      ayahText: maryam2,
      heard: 'زيك رون حمتي ربك عبده وزكريا',
    );
    expect(r.matched, 5);
  });
  ...
  test('the neighbouring ayah is still rejected under the looser match', () {
    final r = TasmeeEngine.compare(
      ayahText: maryam3,
      heard: 'ذكر رحمت ربك عبده زكريا',
    );
    expect(r.matched, 0);
    final back = TasmeeEngine.compare(
      ayahText: maryam2,
      heard: 'إذ نادى ربه نداء خفيا',
    );
  ...
  const nisa1Heard =
      'يوحلنا ستكون بكم الذي خلقكم نفسه وحيناتكم وخلق منها زوجه وذن منه مارجال كثيرا ونسى والتقل الله الذي تسرون به والأرحان إن الله كان عليكم ركيبا';
