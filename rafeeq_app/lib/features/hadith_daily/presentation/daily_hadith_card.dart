...
class _CornerFlourish extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) =>
      CustomPaint(size: const Size(26, 26), painter: _FlourishPainter());
}
...
class _PickedHadithState extends ConsumerState<_PickedHadith> {
  ...
  /// Which way the last move went: +1 forward, -1 back. The incoming card
  /// slides in from that side, the outgoing one leaves by the other.
  int _dir = 1;
          ...
          child: InkWell(
            ...
            child: Column(
              ...
              children: [
                ...
                // «طوّر الأنيميشن في الانتقال من حديث اليوم للذي يليه». The body
                // used to swap in one frame. Now the next hadith slides in from
                // the side it is coming from (mirrored for RTL), fading and
                // settling from a slight scale, while the card's height eases
                // to the new length instead of jumping.
                AnimatedSize(
                  ...
                  child: AnimatedSwitcher(
                    ...
                    transitionBuilder: (child, anim) {
                      ...
                      final incoming =
                          child.key == ValueKey(daily.arabic.hashCode);
                      // Forward in Arabic comes in from the left; mirrored in
                      // LTR; reversed when going back.
                      final side = (rtl ? -1.0 : 1.0) * _dir;
                      final from = incoming ? side * 0.22 : -side * 0.22;
                    ...
                    child: Column(
                      key: ValueKey(daily.arabic.hashCode),
                      crossAxisAlignment: CrossAxisAlignment.start,
                      children: [
