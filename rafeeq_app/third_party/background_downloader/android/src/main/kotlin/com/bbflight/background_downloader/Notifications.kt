...
class GroupNotification(
    ...
    /** RAFEEQ PATCH: fixed `when`, so updates do not re-sort the shade. */
...
object NotificationService {
    /** RAFEEQ PATCH: first-seen time per task, the notification's `when`. */
    private val taskWhen = ConcurrentHashMap<String, Long>()
    ...
    suspend fun updateNotification(
        ...
        val builder = Builder(
            ...
            // RAFEEQ PATCH: one fixed `when` per task and no re-alerting, so a
            // progress update does not move the notification in the shade.
            .setWhen(taskWhen.getOrPut(taskWorker.task.taskId) { System.currentTimeMillis() })
        ...
        if (stateChange || taskWorker.runInForeground) {
            ...
            if (notification == null) {
                ...
                val builder = Builder(
                    ...
                    // RAFEEQ PATCH: see GroupNotification.createdAt.
                    .setWhen(groupNotification.createdAt)
                    .setOnlyAlertOnce(true)
