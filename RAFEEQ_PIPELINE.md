...
| 14 | Library: catalog / offline PDFs / viewer | ✅ | **2026-09-02:** owner lifted the STOP AND ASK (use free sources directly). `lib/features/library/data/book_catalog.dart` — 4 real public-domain classical texts as archive.org PDFs (Riyad as-Salihin, Mukhtasar Minhaj al-Qasidin, Ibn al-Qayyim's al-Fawaid, Ibn al-Jawzi's Sayd al-Khatir); every `downloadUrl` checked with a real `curl -L` GET (HTTP 200, `application/pdf`, exact Content-Length recorded). `_CatalogTab` downloads via the existing `DownloadManager` and `BookReaderScreen` opens the file with `SfPdfViewer.file`. **Live-verified on the emulator end-to-end:** download al-Fawaid → card flips to "فتح" → real PDF renders → airplane-mode ON → still opens from cache and page-scrolls. Honestly a "starting set"; more titles add the same way (one `LibraryBook` + a verified URL). Picked up from a previous session's non-compiling half-edit of `_BookCard`. |
