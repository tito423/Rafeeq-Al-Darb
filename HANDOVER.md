...
<!-- WIP:START -->
**2026-09-13 13:32 — IN PROGRESS — resume here**

the sync plan changed, and NEXT_PROMPT.md now says so loudly - do NOT build the old Drive appDataFolder design. The owner stated what the feature is actually for: «المستخدم يقدر يدخل بنفس اعداداته وتقدمه ومواضع وقوفه في ختمته وعدادات اذكاره من اي تليفون او تاب او حتى لابتوب لما نعمل نسخة للويندوز واللينكس», then «اقصد يبقى دايما في مزامنة» - portable state across devices AND platforms, syncing continuously rather than behind a button. That kills drive.appdata on a fact worth writing down: it is a SENSITIVE scope, so an unverified app re-prompts every 7 days on every device, and the alternative is a Google verification with a privacy policy and a verified domain he does not have. Linux is where Google's and Firebase's Flutter support runs out anyway. New plan, explained to him and recorded: Google for IDENTITY ONLY (email/profile, non-sensitive, no verification, no 7-day expiry) plus a small Worker + D1 on HIS OWN Cloudflare account - the same account already serving the app's books and mushafs, free tier 100k requests/day against an app that needs two or three per user per day, and the data stays in his account exactly as the Drive choice intended. Written down with it: the debounce-on-change / fetch-on-resume / queue-while-offline shape, and the one design question he has not answered - counters should SUM across devices because tasbeeh accumulates, while the khatma position and settings are last-write-wins. Also recorded that he needs three OAuth clients, not two: the two Android ones plus a Web client whose Client ID is what yields an idToken the Worker can verify. Both release and debug SHA-1 fingerprints are in the file, the release one read out of ../Rafeeq-Keys without the password ever being printed.
