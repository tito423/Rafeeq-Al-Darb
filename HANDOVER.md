...
<!-- WIP:START -->
**2026-09-13 03:51 — IN PROGRESS — resume here**

28 Shamela books enter the catalogue, and putting them in exposed four things that were already wrong. The books had been crawled, built and uploaded in earlier sessions and had NO entry in book_catalog.dart, so the app had never been able to open one. Each object was renamed on R2 to a readable key (thdhyr_alsajd_mn_atkhadh_alqbwr_msajd -> tahdhir_al_sajid), and the rename is copy, verify, then delete: the new key is HEAD-ed for size and fetched over the public URL to confirm it answers with gzip's magic bytes before the old key is destroyed and before any entry points at it. FOUR REAL DEFECTS, all found by the checks or by the device, none by analyze: (1) sizeBytes drift - التوسل is 87,662 bytes on the bucket against 87,664 in the manifest, so the script now takes the BUCKET's number, which is what the app downloads, and still aborts on a gap over 64 bytes. (2) Wrong authors - the crawler inherited authorAr from each batch's seed book, so all ten of al-Albani's claimed «الإمام البخاري»; every author is taken from the printed card instead. (3) Three authors split across two spellings each, which the library groups by exactly - «محمد ناصر الدين الألباني، كتاب واحد» sat screens below «الشيخ محمد ناصر الدين الألباني، ١٠ كتب». Same for Ibn Hajar and Abu Nu'aym. Unified to the prefixed house style; the device now shows one row each, 11 / 10 / 10. (4) A DUPLICATE book - the new صحيح السيرة النبوية is Shamela 592, which already shipped as sahih_as_seerah_albani (124,141 bytes against 124,126). Entry dropped and its object deleted from the bucket, so 28 books, not 29. And opening أحكام الجنائز on the device showed a fifth: the sourceLabel printed under the text read «لـالإمام البخاري — بأحكام الألباني» on al-Albani's own book, because the labels had been built from the same inherited field. All 28 rebuilt; the footer now reads «للشيخ محمد ناصر الدين الألباني». One more thing the device showed, unrelated to this batch and older than it: «مكتبتي» split its rows into «مصوّر» and «نصّي», but paths is keyed by book.id while the text section tested book.textDownloadId (<id>_text), which nothing writes - so «نصّي» could never render and every downloaded book, all of them text, sat under a heading calling it a scan. LibraryBook has had nothing but a textEdition for a long time, so the split is gone and the list is flat. VERIFIED on emulator-5554: the three authors each appear once with the right counts, أحكام الجنائز downloads (184.5 KB, matching 188,947 bytes) and opens to real text at 274 pages, its source line names al-Albani, and مكتبتي no longer says مصوّر. Catalogue 247 -> 275 books. analyze clean, 212 tests pass.
