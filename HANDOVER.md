...
<!-- WIP:START -->
**2026-09-12 05:42 — IN PROGRESS — resume here**

batch 11 stage 7 composition verified on emulator-5554, and NEXT_PROMPT.md rewritten. The horizon move from 0.74 to 0.80 is confirmed by eye: «الله أكبر» now sits clear above the mosque instead of the minarets running straight through it, the sun is clear of the centred title with its halo intact, and the sky reads deep navy at the top falling to teal at the horizon. NEXT_PROMPT.md lists, item by item, everything this session wrote but NEVER OPENED on a device - the scrollbar (which changes behaviour on every scrolling screen), the tasbeeh long-adhkar card, the countdown flip, the six new channel cards, the Sources screen's new rows, the ayah card's reciter chip, the prayer notification's city line, the transliteration gating, and above all the download-notification routing, which can only be proven by starting a real download and tapping the real notification - trap 27's shape exactly - plus the four prayer skies other than dhuhr. It also carries the two questions the owner has not answered in three asks: the quota, and whether the ten adhan clips may be removed now that AdhanScene exists.
