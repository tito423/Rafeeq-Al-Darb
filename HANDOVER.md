...
<!-- WIP:START -->
**2026-09-12 05:59 — IN PROGRESS — resume here**

batch 11: more device verification, and one honest not-proven. VERIFIED on emulator-5554: the tasbeeh long-adhkar card opens its sheet with all five adhkar rendering correctly, picking one gives it the counting area as its own patterned card, and three taps counted 33/3 with the gold bar moving and المجموع updating; the Home countdown flips on a tap from الصلاة القادمة الفجر ٢٠:٢٤ counting down to الصلاة السابقة العشاء ٧:٢٠:٠٨ counting up, with the border taking the isha accent; and the adhan settings' sound picker lists twelve adhans, which independently confirms the About screen's live count of 12. NOT PROVEN, and recorded as such in NEXT_PROMPT.md: the scrollbar is no longer permanently on screen (the gold rail that runs down the left edge of the owner's screenshot 7 is absent from the same page 3 now) but I could NOT get it to reappear on scroll, because a synthetic swipe did not move the mushaf page vertically at all - 39 frames of a recording with the left strip unchanged. That has to be retried by hand; if it does not come back, it is a regression I caused. Also written up: ArrowScrollBehavior returns no rail at all when details.controller is null, so the library's lists have none - the owner asked for it on any screen long enough to need it and that is NOT yet true.
