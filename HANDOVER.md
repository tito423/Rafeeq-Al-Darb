...
<!-- WIP:START -->
**2026-09-17 17:55 — IN PROGRESS — resume here**

checkpoint(wip): removed_scholars_test.dart, and writing it found four books I would have mangled. It searches the places where the app itself presents a name as an authority - the catalogue cards, the Sources screen, the channel and site lists, and every string in all seven locales - for all seven names in every spelling they plausibly appear under. On its first run it flagged five catalogue entries, and four of them were «مكتبة ابن تيمية», which is a CAIRO PUBLISHING HOUSE, not the man: al_jami_fi_amthal_al_quran, dhamm_al_malahi, fadail_al_quran_ibn_kathir and nuniyyat_ibn_al_qayyim are books by Ibn al-Qayyim and Ibn Abi al-Dunya and Ibn Kathir that it happened to print. Deleting the publisher to satisfy a string search would have made the provenance line WRONG, which is the worse failure - so the press name is stripped before matching rather than whitelisted per book, which means a fifth book from the same press passes and a book actually BY him still fails. The fifth was mishkat_al_masabih, تحقيق الألباني - and that is a takhrij edition, he graded every hadith in it, so it falls under the same ruling as the 4898 gradings the owner chose to keep. Every exemption is a named string with a reason, and there is a test that the exemptions are still REAL - an exemption describing nothing is a licence nobody is using and the next reader would take it as a statement about the app. Two more failures were code COMMENTS explaining the removals; comments are stripped now, because failing the build over the note that stops the next session putting it back would push the reasoning out of the file. Also fixed: a patch script of mine wrote four literal NUL BYTES into the Dart source instead of the u0000 escape. Dart accepted them and the tests passed over it. analyze clean, 331 tests
