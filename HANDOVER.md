...
| **Can the other 130 be read?** | **Not yet, and it is measured.** CATT encoder-only, run against 10 of the app's own vowelled books as ground truth (3,044 words): **vowel-WER 15.6%**, shadda 16.5%, **case endings 15.4% wrong**. One word in six mispronounced. The tooling is fine — CATT ships ONNX export and is Apache-2.0 — the accuracy on THIS corpus is not. `scripts/evaluate_catt_on_our_books.py` re-takes the number |
