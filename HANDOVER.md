...
<!-- WIP:START -->
**2026-09-08 17:09 — IN PROGRESS — resume here**

Add a flowing page layout for the text mushaf, fix daily-hadith navigation, reverse disclosure chevrons, and restyle the New Muslim guide. Daily hadith: tapping the Home card opened the detail screen with a one-item list, so its Prev/Next buttons had nowhere to go and the swipe PageView had a single page - the navigation was not broken, there was genuinely nothing to navigate to. It now loads the hadith's whole chapter and opens at its index, falling back to the single hadith if the chapter cannot be read. Text mushaf: added a second layout that sets each run of verses as one justified paragraph with inline rosettes, the way a printed mushaf does, alongside the existing boxed-card rows - the owner asked for the page look and then asked for both, so it is a persisted choice (QuranTextLayout) with a toolbar toggle, not a replacement. Verses in the flowing layout are spans rather than widgets, so per-verse tap and long-press are resolved by hit-testing the laid-out paragraph and mapping the text offset back to a verse; scroll-to-playing asks the paragraph for that verse's glyph box instead of using a per-verse key, and the playing verse is washed with a background highlight that rides the text as it wraps. Tapping a verse mid-recitation restarts continuous playback from it, which startContinuous already supported through its supersede token - it just had no reachable affordance in a flowing page. Hadith book screen: dropped the imam name and chapter/hadith counts that repeated under the title, since the collection card in the library already carries both in full. Chevrons: Material's chevron icons set matchTextDirection, so chevron_left was rendering as a right-pointing arrow in Arabic and Urdu - backwards for a forward affordance. Ten disclosure rows now use chevron_right, which mirrors correctly in both directions; the deliberate prev/next pair in single_surah_screen is untouched. New Muslim guide: each card now carries a real Islamic photograph behind a scrim, and the prayer-steps icon is a new hand-drawn SVG of the three postures of a rak'ah instead of Material's meditation glyph, which read as yoga. Every candidate photo was checked by eye first - several plausible-looking ids turned out to be autumn leaves, a fashion illustration and a climbing gym. flutter analyze clean, flutter test 25/25.
