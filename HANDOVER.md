...
<!-- WIP:START -->
**2026-09-05 20:37 — IN PROGRESS — resume here**

P3-44 Nawawi round: fixed 9 miscategorized book categories via new categorize_books.py overrides (minhaj_al_talibin and 4 others to fiqh, 3 to hadith, 1 to aqidah after reading real titles), uploaded all 15 new Nawawi books to R2 with head_object + live curl verification, regenerated and spliced book_catalog.dart to 208 total LibraryBooks, flutter analyze/test clean (21/21), live-verified end to end on a fresh release build (downloaded and opened al_usul_wal_dawabit, real Arabic text rendered correctly via the gzip pipeline). Also flagged a real flutter_local_notifications crash (Missing type parameter, ScheduledNotificationBootReceiver) found live during this pass, plausibly related to the owner's notification complaints, left for its own dedicated fix rather than guessed at here.
