...
<!-- WIP:START -->
**2026-09-17 17:45 — IN PROGRESS — resume here**

checkpoint(wip): 13 of the 27 new books are on the bucket, and reading the built files caught two bad editions before either shipped. iqtida_al_ilm_al_amal: the ONLY printing Shamela has of al-Khatib al-Baghdadi's book is al-Albani's edition. The builder drops the apparatus so the file is the author's text, but sourceLabel would still have to name the printing - and naming him is exactly what was just undone - so the book is dropped and جامع بيان العلم وفضله covers the same ground. maqasid_al_riayah: I catalogued it from its title as al-Muhasibi's, and the built file's OWN edition card says العز بن عبد السلام ت 660. It is his abridgement of al-Muhasibi's الرعاية, not al-Muhasibi's book. That is trap 17's family and it was caught only because build_book_text prints the edition card and I read it rather than trusting the title I searched for. Label corrected, book rebuilt, re-uploaded. The uploader was also fixed mid-run: it overwrote _uploaded_sizes.json instead of merging, and since it runs in batches as builds finish and write_catalog_entries reads that file for every sizeBytes, a later batch would have silently dropped the earlier one's sizes and those books would have been skipped with no error. sizeBytes is what R2 answers on a HEAD after the put, never the local file's length - they are equal here but the rule is that isBookDownloaded compares against the hosted object. CONTENT-LICENSES.md now carries the whole curation pass: what each of the seven names actually owned (measured), why al-Albani's 4898 gradings stayed and his name is gone from everywhere else, that القرني in the tafsir is أويس القرني the Tabii and was not touched, the stale Ibn Baz row, the three replacement sites with their own titles and byte counts, and why الاعتصام is absent while الموافقات is in
