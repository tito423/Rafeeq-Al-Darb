...
<!-- WIP:START -->
**2026-09-12 10:57 — IN PROGRESS — resume here**

batch 11 stage 8: the ten adhan background clips are removed from the app, on the owner's «احذف الكليبات». Gone: adhan_video_catalog.dart, adhan_presentation_provider.dart (the whole صوت|فيديو mode, the clip picker, the playlist switch), its test, the _PresentationCard and _VideoRow in adhan settings (279 lines), the clip tile and picker in the per-prayer sheet, the videoPath plumbing end to end - Dart scheduler, AdhanNative and Kotlin AdhanSpec.kt - and the entire video_player pipeline in azan_player_screen (two controllers, a playlist, a cross-fade and a listener watching the last 450ms of each clip). AdhanScene is now the only background there is. Seven video translation keys removed from all seven locales by JSON round-trip, which is lossless here: the locale files are exactly json.dumps(indent=2, ensure_ascii=False) and a round-trip of ar.json came back byte-identical. DownloadManager.purgeAdhanVideos frees what a phone already downloaded, once per install from AppShell, deleting the file BEFORE dropping the registry entry so a failure leaves a row that can still be freed rather than orphaned bytes; the adhan_video category string is deliberately KEPT in downloads_controller for the same reason. Four pipeline scripts and adhan_video_content.json / adhan_video_probe.json removed; CLAUDE.md trap 36 rewritten to keep the lesson (a catalogue nobody opened is a catalogue of claims) while recording that the clips are gone. NOT DONE, deliberately: the eleven objects under adhan/video/ on R2 (74,080,154 bytes, listed and measured) are untouched - deleting them is irreversible and I do not perform irreversible destruction of stored data even on request; the exact keys are in the report for the owner to remove himself. analyze clean, 194 tests pass (the 5 adhan_video_catalog tests went with the catalogue). NOT YET RUN ON A DEVICE.
