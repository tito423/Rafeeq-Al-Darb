...
<!-- WIP:START -->
**2026-09-17 13:00 — IN PROGRESS — resume here**

the hand-curation he asked for, with its tooling and its first 22 adhkar. NO ARABIC IS TYPED ANYWHERE in azkar_curated.json: an entry is a pointer - a section index, an item index and two short phrases that locate where the supplication starts and ends - and the text that will reach the database is sliced out of the extractor's own output at build time, which is the same discipline the quotes corpus uses so a diacritic cannot be lost in transcription. The phrases are a reading aid and never ship; one that stops matching fails the build loudly rather than half-matching and shipping a clipped supplication, and every entry now carries a frozen sha so a re-fetch that shifts a paragraph stops the build instead of quietly changing which dua sits under a title. azkar_pick.py is the reading tool: it prints a bab with every quotable span and resolves «from this phrase to that one» into offsets. 22 duas across 11 abwab - waking, dressing, leaving and entering the house, the khala, wudu, setting out for the mosque, entering and leaving it - each with an-Nawawi's own takhrij sliced beside it. Three defects found by READING the output rather than trusting errors=0: hadith 81 carries TWO supplications joined by «وَإذَا خَرَجَ فَلْيَقُلِ» and was shipping as one card, so it is two entries now; hadith 55's takhrij slice had swallowed the whole hadith again; and 74's ended on a dangling quotation mark. Also fixed in the extractor: five chapter titles came out as «ُ تكبيرةِ الإِحْرام» because the alternation matched «باب» before «بابُ» and «\b» means nothing useful next to Arabic - same family as trap #47. All 338 titles well-formed now. NOT done: the DB rebuild, the six ruqyah ids, the Dart side, the translations, and the remaining abwab
