...
<!-- WIP:START -->
**2026-09-05 13:42 — IN PROGRESS — resume here**

P3-43 item #10 done + live-verified: location permission has a real direct action now. Home's location-denied card (_MessageCard) was static text with no tap action - it gained an optional action button, wired in _PrayerCard (now a ConsumerWidget) to PrayerController.refresh(), which already calls Geolocator.requestPermission() internally via LocationService.getCurrentPosition() - no new permission path needed, tapping the button genuinely triggers the real OS prompt and continues straight into fetching real prayer times on grant. +1 key x 6 locales. Second half - granting via the OS Settings app directly (not through the new button) needed AppShell (already a WidgetsBindingObserver) to recheck on every resume, but only refetches when the last known state was genuinely locationDenied, so this doesn't add a spurious call on every ordinary resume. flutter analyze/test clean (21/21). Live-verified on emulator-5554: revoked location permission via adb, relaunched, saw the real Allow/Don't-allow OS dialog, denied it, confirmed the Home card now shows a real green Enable Location button (not static text) and tapping it doesn't crash - Android's own no-more-prompts-after-denial policy explains why a second dialog didn't reappear immediately, expected OS behavior not an app bug.
