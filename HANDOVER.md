...
<!-- WIP:START -->
**2026-09-06 15:18 — IN PROGRESS — resume here**

P3-53 Part 4: new Colored Tajweed edition (raster/image-scan path). Sourced the classic Dar Al-Maarifa colour-coded tajweed mushaf (604 real pages, verified) and rehosted it first-party to R2 at mushaf/tajweed/NNN.jpg via new scripts/r2_upload_tajweed_pages.py (streams GitHub->R2, resumable, head_object-verified). App side: MushafEdition gained imagePath + isRaster + imagePageUrl; AppConfig.mushafImageUrl builds the R2 URL from contentBaseUrl. MushafPageView branches to a raster renderer for image editions - disk-first (offline) via MushafPageService.cachedImageFile, else cached_network_image with a capped memCacheWidth (also satisfies the Part 3 cached_network_image ask), zoomable, no polygon/coords/ayah-tap layer (a finished coloured scan has none). MushafPageService.prefetchEdition gained an imagePath branch that downloads JPGs to the same per-edition dir the SVG cache uses, so cachedPages/cacheSizeBytes/clearCache/progress all cover raster editions unchanged; mushaf_download_tile passes edition.imagePath. quran_screen coerces raster editions into image mode and hides the text/image toggle + text-only controls (a scan has no reflowable form). editions.json: added tajweed_color (Hafs numbering, sciences_aligned). QuranBookCoverThumbnail: maroon (نبيتي) leather for tajweed + medallion override 'تجويد' (avoids a 2nd 'حفص'); reserved a black-gold Shamarly slot. flutter analyze clean, tests 21/21. Live-verified on emulator-5554: Tajweed appears in Choose-a-mushaf with its maroon cover; selecting it hides the mode toggle and renders the colour-coded Al-Fatiha page 1 streamed from R2. Note: Shamarly still unsourced (the GitHub app repo's shamerly.zip is only 2MB, not the 604 pages) - pending real page images to upload with the same script. Tajweed R2 upload finishing in background.
