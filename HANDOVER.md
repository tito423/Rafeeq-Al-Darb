...
<!-- WIP:START -->
**2026-09-12 05:03 — IN PROGRESS — resume here**

batch 11 stage 6: the keyboard flicker is found, and it was not the keyboard. SystemChrome.setEnabledSystemUIMode is PROCESS-WIDE, and QuranScreen set immersiveSticky for its full-screen mushaf while being a kept-alive tab in AppShell's IndexedStack - built on the first frame, never disposed - so the moment _restoreState read a stored ملء الشاشة of true the WHOLE app ran immersiveSticky for the life of the process and dispose's restore never ran. immersiveSticky peeks the bars back on any interaction and hides them again, so every keyboard open resized the window twice: that is the owner's فليكر حاصل في أي حتة في التطبيق. Measured, not guessed - screenrecord of the library search screen at 15fps shows the keyboard bouncing up, part-way down and up again with the status bar flashing with it, and dumpsys window on that screen reported type=statusBars visible=false on a screen that asks for nothing of the kind. The mode now follows activeTabProvider, the same seam the Qibla compass already uses to pause its sensor; test/immersive_mode_scope_test.dart guards every call site and was PROVEN to fail on the broken code (reintroduced the unguarded call and watched it report the exact line). Also in this stage: the RGB backdrop is now Islamic - a girih khatim lattice tessellated across the screen with its interlacing diamonds, a turning rub el hizb medallion and one travelling lamp sweep, gold on deep navy, replacing the aurora of coloured rings, colour band and forty motes; the three full-screen MaskFilter blur passes it used to do every frame are gone and the lattice Path is built once per size and only translated. And the tutorial's skip button, which did nothing at all: _finish called maybePop, which consults the route's PopScope, and this screen sets canPop:false so the request came straight back to _finish - the tour could only be left by finishing it. pop() now, with a re-entrancy guard. analyze clean, 199 tests pass. DEVICE: skip verified working, About verified showing v3.19.0 and live counts (6 mushafs, 45 languages, 176 reciters, 12 adhans, 20 methods, 9 collections, 67153 hadiths of which 45219 graded, 228 books, 13 channels, 7 locales), du'a card verified rendering, RGB lattice verified on Home and Library.
