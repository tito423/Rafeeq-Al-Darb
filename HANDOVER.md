...
<!-- WIP:START -->
**2026-09-17 17:16 — IN PROGRESS — resume here**

checkpoint(wip): Ibn Taymiyyah's 60 books out of the library, and the count was measured before he was asked, not after. He asked for seven names removed; six of them own NOTHING in the library, and the seventh owns 60 of its 248 entries - 24 percent - so I put the number in front of him rather than deleting a quarter of the library on an inference. He chose all sixty plus Ibn al-Qayyim's أسماء مؤلفات ابن تيمية, which is an index OF those sixty and would have been a card pointing at books that are gone. The al-Albani question went the other way and for a reason worth recording: his name is on 4898 hadith gradings in hadith.db, and that is isnad criticism with a named grader, which is what CLAUDE.md 1.2 requires - stripping the name would have left 4898 hadiths reading غير مذكورة, so the takhrij stays and only the links, books and descriptions go. Before touching anything I searched every course, quote, guide and catalogue for the 61 ids: nothing outside book_catalog.dart and three book_desc keys pointed at them, so there is no dangling pointer - an id is a plain string and analyze cannot see one. editor_notes_removed_test.dart pins two counts deliberately so a change has to be explained: flagged 47 -> 29 (18 of the filtered books were his) and the catalogue 248 -> 187. R2 deletion is written but NOT run yet - the script refuses if any id is still in the catalogue, because the safe order is catalogue-first: a half-run then leaves files nobody points at, not cards pointing at files that are gone. analyze clean, 319 tests
