...
<!-- WIP:START -->
**2026-09-17 16:06 — IN PROGRESS — resume here**

أذان الشيخ ربيع القاضي (azan13) out of the picker, temporarily and reversibly. Removing it from adhans.json alone would not have been enough: a choice already saved under adhan_choice_v1_<prayer> keeps pointing at the id, and while the scheduler falls back to the first adhan the settings dropdown ASSERTS on a value that is not among its items. There is already a mechanism for exactly this - removedAdhanIds, built when two Alafasy adhans were dropped - so azan13 joins it and a stale choice reads as «no choice» instead of crashing the sheet. Because he said مؤقتا, the mp3, the measured loudness and the phrase timings are all left in place: restoring it is one line in adhans.json and one id out of that set, with nothing to re-measure. That deliberate asymmetry broke adhan_audio_test.dart, which asserts the catalogue and the measurements match BOTH ways - correctly, because a measurement with no catalogue entry is normally dead data. It now exempts ids declared in removedAdhanIds and still fails on an undeclared orphan. removed_adhans_test.dart is new and holds three things: nothing marked removed is still on offer, azan13 specifically is out of the picker AND declared, and every adhan still offered has a file behind it - because a picker entry with a missing asset is silence at the adhan. analyze clean, 317 tests
