...
<!-- WIP:START -->
**2026-09-12 05:34 — IN PROGRESS — resume here**

batch 11 stage 7: AdhanScene - the adhan's background is painted now, not filmed, and it is verified on emulator-5554. Five skies, one per prayer (fajr pre-dawn blue with a setting crescent, dhuhr pale gold high in deep teal, asr amber, maghrib rose with the first stars, isha deep indigo), a mosque in silhouette with dome, two minarets, galleries and finials, lit along the edge the sun or moon is actually on, hills behind it and the light's reflection on the ground. Every complaint the owner made about the clips is a property of using a clip: 640x360 upscaled 6.7x on a 1080x2400 phone (trap 37), a 20s clip under a 4-minute adhan looping twelve visible times, and somebody else's licence - a painted scene has none of those, is resolution-independent and weighs nothing, and its clock counts 30 minutes so no period is short enough to read as a repeat. THE SYNC IS REAL AND WAS SEEN: a ring of sound leaves the minaret on every phrase, driven by the same _activeIndex the subtitle line uses, which comes from the recording's own measured onsets in adhan_phrase_timings.json. Recorded 40s of the preview at 6fps and the frames show it - a small ring born at the finial while «حيّ على الصلاة» is up, grown large by the time the text turns over to «حيّ على الفلاح», and a new small one born at that instant, the two overlapping and fading as designed. Three things fixed by looking at it: the scrim was built to make white text survive ANY footage (0xCC at the top) and crushed the sky flat and turned the sun into a brown smear behind the title - lightened to protect only the header and button rows; the sun sat behind the centred title in every prayer - moved off the axis and down; and the minarets reached straight through the phrase text - horizon moved from 0.74 to 0.80 and the minarets shortened, which is also where a mosque on a horizon belongs. NO clip has been removed - the ten stay until the owner answers. analyze clean, 199 tests pass.
