...
<!-- WIP:START -->
**2026-09-08 15:40 — IN PROGRESS — resume here**

Fix broken library tab labels, hadith collection cards, Quran text-mode button, and add manual prayer/Hijri corrections plus new About and Sources screens. Dropped the Korean and Kurdish Quran translations at the owner's request - each was missing one verse upstream - and deleted their objects from R2, so all 45 remaining languages are complete 6236-verse translations and the uploader now rejects anything short again. Found and fixed four translation keys that were referenced in code but defined in no locale at all: the library's third and fourth tabs were literally rendering the strings library.tab_channels and library.tab_websites on screen, now Dawah Channels and Islamic Websites in all six locales, and the tasbeeh haptic toggle had the same problem. Hadith collection cards rebuilt: the compiler's full name was clipped to one line so Sunan al-Darimi showed a fragment of its imam's name with no way to read the rest; the name now wraps, a short factual note on each imam was added (new hadith_imam_bios.dart, birth and death years and what the collection is known for, no gradings or rankings), and the hadith and chapter counts are their own pills. The Quran reader's text-mode button was hidden on raster printings, which left a reader who had picked one with no way back to the text reader; it is always shown now and switches back to the default text edition when leaving a scan. New PrayerAdjustments store and screen reachable from the Prayer tab: manual Hijri date offset in days, per-prayer minute offsets, and the calculation method moved here from Adhan settings, where it never belonged - that screen is about how the adhan is announced, this is about when a prayer is. Offsets are applied in PrayerController before scheduling, so the alarm fires at the time shown on the card, and the Home header's Hijri line honours the same offset. About is now its own screen with a staggered intro animation, a shimmering author credit for Tito Abo Malak, the version, and a capability list written from what actually ships. Sources became its own screen too, grouped by what each source feeds with a tappable row per source, replacing a single run-on line of dot-separated hostnames that credited nobody legibly. Morning adhkar now use a dawn-lit mosque image instead of the previous unrelated photo. All new copy is really translated into the six locales rather than left in English. flutter analyze clean, flutter test 25/25.
