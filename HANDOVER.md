...
<!-- WIP:START -->
**2026-09-13 03:22 — IN PROGRESS — resume here**

the bundled mushaf is OUT - the release APK drops from 324.6 MB to 226.7 MB. The owner asked for it directly: «احذف المصحف المصور اللي بيتحمل وينزل مع التطبيق out of box». P3-41 had shipped all 604 SVG pages of the default edition inside the APK so it was readable the moment the app installed; on disk they were 350 MB and in the compressed APK about 98 MB, which was the single biggest thing in the download. MEASURED, not estimated: assets/mushaf/hafs_kfqc was 350M over 604 files, the release APK was 324.6 MB before and is 226.7 MB after, and the debug APK went 519 to 465 MB. VERIFIED on emulator-5554 after a clean reinstall: «وضع المصحف» opens and page 1 of سورة الفاتحة renders correctly - and it can only have come over the network, because nothing is bundled any more and hafs_kfqc never wrote to the disk cache while it was. No new download code was needed: svgForPage already fell through to cache-then-network whenever an asset was missing, which is exactly the path the four non-bundled editions have always taken. Two code changes, both deliberate: _kBundledMushafEditions is now an empty set (kept rather than deleted so bundling again is one id and no other change), and _bundledSizeBytes lost its hafs_kfqc entry - leaving 365010757 there would have made «التنزيلات» report 348 MB of storage that is not on the device. One consequence the owner should know and it is written in NEXT_PROMPT: the default mushaf now depends on raw.githubusercontent.com (quranpedia/quran-svg, pinned commit) the way every other edition already did - probed 200 with image/svg+xml on pages 001, 302 and 604 - and R2 does NOT mirror it (404). Mirroring those 604 files to the bucket is the obvious follow-up. analyze clean, 212 tests pass.
