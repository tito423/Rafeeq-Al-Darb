...
<!-- WIP:START -->
**2026-09-17 01:52 — IN PROGRESS — resume here**

the Hajj guide was printing Ibn Baz's manual page by page, which is reproducing it however the citation reads, so it now reads al-Nawawi's al-Idah fi Manasik al-Hajj instead - the classical pilgrim's manual, author died 1277, and it turned out to have been sitting in the library the whole time under al_idah_fi_manasik_al_hajj_wal_umrah. Every one of the eighteen ranges was re-read off the book's own chapter openings (al-Bab al-Awwal p45, the miqat p113, tawaf p206, sai p251, Arafat p263, Muzdalifah p295, the day of Nahr p309, Tashriq p357), and the hosted copy was fetched and checked to be the same build the ranges were read off. Two changes the book forced: مسجد قباء loses its step because al-Nawawi sets no section for it and a card must not point at nothing, and العمرة gains one because his Bab al-Rabi is a chapter of its own. And a filter, because 472 of the 1,576 body paragraphs from p45 on are al-Ifsah, a modern commentary that Shamela leaves in the ordinary flow rather than in hamesh. Ibn Baz's file is off the bucket now too
