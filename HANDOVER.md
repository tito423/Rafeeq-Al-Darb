...
<!-- WIP:START -->
**2026-09-09 11:59 — IN PROGRESS — resume here**

Mushaf Qatar: SIX corrupt pages found in the source and four of them repaired, plus a per-page ayah fit for all 604. I have to correct my own claim again - I said '604 pages verified' when what I had verified was that all 604 are FETCHABLE. They are not all legible. Page 210 is a flat green block over most of the sheet in the archive.org copy, page 5 a pink wash, page 167 a grey wash, page 323 a torn orange band. Found by scanning every rendered page for a large flat coloured area, after the polygon overlay on 210 came out four times too big and the raw embedded JPEG turned out to be corrupt in the source itself. Repaired from a second archive.org copy, and that this is recovery rather than mixing sources is PROVEN: 598 of the 604 embedded images are byte-identical between the two copies, and each of the six that differ has the same byte length in both - the signature of a corrupted copy. 241 and 385 also differ but both copies of each are clean (the difference is confined to glyph edges), so those keep the primary. The second copy draws a www.Quranpdf.blogspot.com watermark as TEXT over every page, which the first render baked into the four repaired pages; it is redacted before rendering, images untouched, and the top strips of all four now match their neighbours. All 604 re-uploaded and re-range-requested. Ayah fit: the printed frame is the fiducial, since the frame keeps a constant SIZE (sd under 0.5%) while its position slides up to 3% of the page width - 26px, two letters - which no single affine could absorb. Every body page yields exactly 15 ink runs and the Hafs slots cluster to exactly 15, so slot k matches run k with no guessing. 571 pages fitted directly with a median residual of 4px on a 1227px page, 33 filled from the frame (their frame-relative parameters agree across 567 pages to sd 0.002), and 2 whose frame box was implausible borrow the nearest fitted page. Proof overlays rendered and looked at on 1, 2, 50, 210, 419, 601, 604
