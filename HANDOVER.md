...
| **HANDOVER 2026-09-26 ~11:30** | **Released: still v3.63.7** (tag `c6f8d3ac`); `master` `1bc93dc8`+ far past it. Verified now: `flutter analyze lib test` clean, **595 pass**; hosted 10/10 → 206 (hadith.zip 22,235,941 · tafsir_al_tabari 7,551,247 · madinah_qc 001 40,445 / 604 111,649 · am.json.gz 336,066 · sciences v1 33,239,511 · sciences/v2 32,146,611 · cities.tsv.gz 5,080,747 · fp_ms.onnx 187,215,347 · privacy.html 11,903). Measured: 7 locales × **1,827** keys; **1** mushaf edition (madinah_qc); library **239**; hadith.db 109,731,840 B, **67,153** hadiths, **45,219** graded; pubspec `3.63.7+72`; local signed APK 275,449,345 B (11:24, not installed anywhere). **This session (owner: the app must be a tablet sideways, «كانه تاب شغال بالعرض»):** splash shows the whole portrait clip sideways (WholeClip, also the preview); side rail `SideTabs` (7 tabs share the height); TwoPaneScroll on Home/Prayer/Downloads; PairedListView/PairedColumn (2-3 a row) on More, settings, reciters, tajweed, hifz, ruqyah, dedications, library lists, hadith, encyclopedia, channels, websites, adhkar sections; readableInsets (680 dp) on form pages; showFittedSheet on 18 sheets (were clipped at 9/16); sciences sheet 2 columns (i'rab full height); book reader actions in a side column; tab state kept across rotation (`_tabsKey`); mushaf controls on tap sideways (owner reversed the portrait-only rule). **Bugs found and fixed:** text-mushaf ayah markers REVERSED within each RTL line (Flutter 3.38.7 engine gives RTL placeholder boxes in visual order - now painted from each verse's own words, `ayah_marker.dart`, test `rtl_ayah_marker_order_test`); makharij diagram off its drawing (AspectRatio under a tight width); dark icons on dark app bars (AppBarTheme beats foregroundColor); Latin digits in Arabic (Bukhari chapters, download counts, reminder times). All seen on emulator-5554 (Arabic, sideways and upright) EXCEPT the 11:24 build (More 3-a-row, no pinned surah header in full screen) - NOT installed. **Open (owner, new, not started):** tasbeeh sideways fill the height; side rail closer to the notch edge + use the screen on every page; Home analog clock + prayer card to shrink and fit on any tablet / smart screen; then report + publish (he asked). Exact list: `NEXT_PROMPT.md`. |
| **HANDOVER 2026-09-26 ~02:10** | **Released: still v3.63.7** (tag `c6f8d3ac`); `master` far past it, nothing of 09-25 evening or 09-26 released. Verified now: `flutter analyze lib test` clean, **587 pass** (594 minus the 7 Corpus-parser tests deleted); hosted 10/10 → 206 (hadith.zip 22,235,941 · tafsir_al_tabari 7,551,247 · madinah_qc 001 40,445 / 604 111,649 · am.json.gz 336,066 · sciences v1 33,239,511 (kept for 3.63.7) · **sciences/v2 32,146,611** + its GitHub content-mirror asset · cities.tsv.gz 5,080,747 · fp_ms.onnx 187,215,347). Measured: 7 locales × **1,827** keys; 1 mushaf edition; hadith.db 109,731,840 B; pubspec `3.63.7+72`; local signed APK 275,416,577 B. Library count NOT re-measured (untouched). **I'rab DONE (owner's order):** al-Da'as «إعراب القرآن الكريم» replaces the Corpus word cards; 27 Shamela-damaged sections transcribed from the print; 6 Qur'an quotes corrected to the mushaf + 3 commentary errors (in all 4 copies of the book) fixed from الجدول/درويش/الإعراب الميسر, each marked «[تصويب عن …؛ وفي الأصل: …]», evidence in `scripts/evidence/`, record `scripts/irab_daas_quran_corrections.json`; 215 «سبق إعرابها» shown with the earlier i'rab framed under the book's line. SEEN on emulator-5554 (build 01:19, before the commentary fix): 2:5, 2:31→2:23, Sources. **Owner's Xiaomi** (23078PND5G, Android 16, `BYKRKJPRC6O7FMHU`): signed build (01:58) uninstalled + reinstalled clean at 02:02:48 on his order, NOT opened after. **Open:** orientation on the Xiaomi (owner: «فيه العجب في الأورينتيشن» - seen: auto-rotate ON, app follows sensor ROTATION_90 while the launcher stays portrait; splash never captured - screenrecord blocked by trap 46, redo from PowerShell); 222 unproved references must be CHASED in other i'rab books (owner rule), not parked; Shamela mid-sentence line breaks; Avast on the phone showed `MalwareFoundActivity` in recents - not investigated. |
| **HANDOVER 2026-09-25 ~23:50** | **Released: still v3.63.7** (tag `c6f8d3ac`); `master` is far past it, NOTHING of today's evening is released. Verified now: `flutter analyze lib test` clean, **594 pass**; hosted 9/9 → 206 (hadith.zip 22,235,941 · tafsir_al_tabari 7,551,247 · madinah_qc 001 40,445 / 604 111,649 · am.json.gz 336,066 · quran_sciences.zip 33,239,511 · geo/cities.tsv.gz 5,080,747 · fp_ms.onnx 187,215,347 · store/google_play/…/01_home.png 734,293). Measured: 7 locales × **1,821** keys; library **239**; hadith.db 109,731,840 B; tour pictures **238** (7 × 34); pubspec `3.63.7+72`. **Done since 15:50 (code, most seen on emulator-5554):** plan items 1,2,5-14; ayah text + font = King Fahd Complex hafs v18 (seen; owner's phone NOT yet on it); tour = pictures of the 8 MAIN screens whole, 7 languages (seen, quick 9/9); Play Store gallery out of the app, originals on R2 `store/google_play/` + in git. **Found:** text-mushaf lines drawn REVERSED on the emulator under Impeller (both fonts; Skia correct; layout correct) - owner's phone (3.63.7) shows the RIGHT order → emulator-only, never judge mushaf order on the emulator. **I'rab (owner's order):** source = al-Da'as «إعراب القرآن الكريم» (Dar al-Namir/Dar al-Farabi 1425; Shamela's «المنير» is a typo). Two digital copies exist and BOTH are damaged (Shamela: dropped first letters, glued words; KSU/e-quran: made from Shamela, hand-repaired with errors, own notes inserted, text under the wrong ayah). Judged against the PRINTED book (archive.org `i3rb-krn-d3s`, Windows ar-SA OCR of all 1,418 pages): 3,612 sections final from Shamela (54,434 8-word chunks vs print, 24 doubtful, all read and correct); **27 damaged sections are being transcribed from the print - 5 done**. Not yet in the app. Details + exact resume: `TASK_FOLLOWUP.md`. |
| **HANDOVER 2026-09-25 ~15:50** | **Released: v3.63.7** (tag `c6f8d3ac`, asset 270,769,028 B, sha256 `b3c22dc8…` = local). Since 3.63.1 (all seen on emulator-5554 only): unzip of packs off the UI isolate (the owner's Xiaomi showed «isn't responding»), mushaf download retries with backoff instead of dying on a screen lock, «لاحقًا» on the first-run downloads page and that page reachable again from Downloads, large-font tasbeeh/library tabs/Home PM, the owner's final app icon (his 1024 image without prayer beads, cropped to the ring). Verified now: `flutter analyze lib test` clean, **588 pass**; hosted 8/8 → 206 (hadith.zip 22,235,941 · tafsir_al_tabari 7,551,247 · madinah_qc 001/604 · am.json.gz · quran_sciences.zip 33,239,511 · geo/cities.tsv.gz 5,080,747 · fp_ms.onnx 187,215,347). Measured: 7 locales × **1,833** keys; library **239**; hadith.db 109,731,840 B; pubspec `3.63.7+72`. **Open:** the owner's review of 10 videos + 11 screenshots from his phone → 5-stage plan in `NEXT_SESSION_PROMPT.md` (back arrows, book removal, lying download state, silent recitation button, missing `quran.stop` key, sciences-pack false «download», move prayer location, delete the New-Muslim guide, More accordion, tour, theme-blind cards, ayah highlight, performance, i'rab source audit «على = جمع», qibla on a real phone). Not started. |
| **RELEASE 2026-09-25 (v3.63.1, on the OWNER'S PHONE)** | First session run on a real phone: Honor BRP-NX1, Android 12 (`AB3S6R4C04016607`). v3.63.0 was published at 10:33 and replaced by **v3.63.1** (`pubspec` `3.63.1+66`, About v3.63.1) after the phone showed a real bug: **28 library books had never been measured for diacritisation** (default 0) - «إحكام الأحكام», visibly vowelled, refused «Listen» with «only 0%». Re-measured all 239 from the hosted text (Ihkam 83.4%; 7 of the 28 now speakable). **Largest system font (1.45, bold)** fixed: tasbeeh counter shrank to a dot in portrait, library sub-tabs non-uniform, Home slide clipped «PM». **Seen on the phone:** install over the old build, onboarding permissions (one ask each), Home Dubai times, manual location Tanta (Dhuhr 1:48 PM = hand calc 09:48 UTC; qibla 259 -> 138) and back to automatic, location switch OFF -> 0 Google dialog launches; adhan (Test, real alarm path) pauses and resumes ruqyah, device files and the enhanced book-reader voice (same player resumes); book reader stops ruqyah; tajweed plays on the shared player; theme and language switch while playing; background + back while playing; a 108 MB download finished while the app was in the background; enhanced voice downloaded (260.7 MB). **Not done:** largest font in LANDSCAPE (rotating the owner's phone is his), the no-cached-fix «Enable location» path (needs cleared app data), city name ar->ur first-switch delay (cause unknown). analyze clean, **588 pass**. Owner's phone left as he set it: font Huge/Bold, location OFF, app Light + English; test downloads kept (Alafasy + Abkar ruqyah, enhanced voice). |
| **SESSION 2026-09-25 02:30-05:15 (not released)** | Owner orders A (contrast) and B (manual prayer location) **done and seen on emulator-5554, build 15 = master**. **B:** Adhan settings > «Prayer location»: automatic / world list / phone geocoder / coordinates. The GeoNames cities1000 list (171,075 places, CC BY 4.0) is now **BUNDLED** (`assets/data/cities.tsv.gz`, 5,080,747 B - the owner: «دول خمسة ميجا بس»; APK 253.1 -> 258.0 MB), unpacked once on first open (5.3 s measured). Seen offline (airplane + location off): «Tanta» -> Tanta, Egypt first (~2.8 s); Home times follow (Sunrise 7:46 = Dubai 6:09 + 97 min longitude); Makkah by coordinates; back to automatic. **Two older bugs found and fixed:** `PrayerTimesService` answered from a cache keyed by DATE ONLY (new place / method / madhab ignored until midnight - test `prayer_times_place_test.dart` fails on the old code); the Qibla (kept-alive tab) never saw a place change - now 258° -> 138° = great-circle calc. **A:** crawl + measured scan in light, dark, RGB, every flag eyed. Fixed: ~56 flat-gold icons on theme surfaces -> `goldText`; the reading-options card; the player's selected tab; theme chip icon on the wrong container pair; chips drawing a check over their icon; **RGB filled buttons white on #22E0C6 = 1.67:1** -> `onPrimary` picked by measured contrast (`theme_on_primary_test.dart`). Left as is, on purpose: disabled controls, decorative page dots, the Hajj map line. Also seen: Fajr adhan fired on time at 04:50 on build 14. `flutter analyze` clean, **588 pass**. **Emulator note:** `emu kill` + restart resumes an OLD snapshot (app and settings revert) - reinstall after every restart. |
...
| **v3.59.0 (2026-09-24)** | `pubspec` `3.59.0+61`, About v3.59.0. APK `dist/RafeeqAlDarb-v3.59.0.apk` 264,911,428 B, sha256 `1ec92b37…66db29`, versionCode 61, arm64-v8a/armeabi-v7a/x86_64, support link in all three libapp.so, signed CN=Rafeeq Al-Darb (9+). `flutter analyze` clean, **551 tests pass**. **Library compatibility:** permission_handler 13 (from 81d41366) needs AGP 9 and broke the release build — back to **12.0.3** (android 13.0.1, AGP 8). All 30 Android plugins audited for AGP/Kotlin/compileSdk/minSdk; that was the only AGP-9 one. **Recitations measured from this machine:** all 35 per-ayah reciters × 7 ayahs (+R2 for the 3 mirrored) = 266 range requests, 266 good; mp3quran live catalogue 241 reciters / 287 moshafs, first+last listed surah each + both R2 surah mirrors ×114 = 801 requests, 1 bad: عبدالله البريمي (moshaf 57) An-Nas 404 (1 of his 69; upstream). **Every tab and setting walked on emulator-5554** (Home, Quran text+paper+jump+topic+ayah sheet tafsir/translation/i'rab, Prayer/Qibla/adhan preview/method/offsets/Hijri, Azkar all categories + reminders, Tasbih, Library all tabs + Nasa'i 35.2 + book download/open/TTS, player stream/download/device files, tajweed 3 levels, Hajj+Umrah, hifz+tasmee, khatma, dedications+share, new-Muslim, downloads, focus mode, tour skip, all reminders armed & cancelled in AlarmManager, every settings section, About/Sources/Privacy, English UI). **Fixed and re-seen on device:** i'rab lemmas «ؤلت;ِن» (10,040 HTML-escaped + ^/# unmapped); daily-hadith bare grade (§1.2); Arabic-digit clock 1.06:1 on light; finished ayah replaying after the adhan/any focus loss; tasbih «تصفير الكل» dead with total>0; surah player 404 said «check connection»; «كل الجمعة». **Not seen / not done:** Quran word search, library search (adb cannot type Arabic); the 260.7 MB enhanced reader voice download; home «تفعيل الموقع» refusal-for-good path (needs a device with no cached fix); anything on the owner's phone. The emulator itself crashed 3× this session (host-side: `UpdateLayeredWindowIndirect failed`, a segfault) — the same step afterwards ran clean. |
| **RELEASED 2026-09-23 night (v3.58.0)** | tag `v3.58.0` = `ae161ef0` = HEAD at publish; asset `RafeeqAlDarb-v3.58.0.apk` 264,530,356 B, sha256 `5bb3f5f0…2876db`, versionCode 60, minSdk 24, arm64-v8a/armeabi-v7a/x86_64, support link in all three libapp.so; download 206. v3.57.0 + tag deleted, v3.51.0 kept. Seen on emulator-5554: installed -r over the debug build (data kept), no FATAL, «ادعم التطبيق» present, About card v3.58.0 and «٣٥ قارئًا». Contents: the 157 dead reciters removed (list 35, whole-mushaf checked), failure message with host+reason, R2 mirror primary (Alafasy/Maher/Minshawi murattal per-ayah + Basit/Maher surahs, 18,936 objects counted) with the origins as fallback, default reciter Minshawi MURATTAL (proven playing from R2 in logcat), focus mode hifz card, theme choice on the first screen, onboarding + sign-in sheet follow the theme, «المصحف الورقي», pinned download row. **Not seen:** anything on the owner's phone; whole-surah stream/download from R2 on a device. Tests 548 pass. |
| **HANDOVER 2026-09-23 (evening)** | `master` pushed at `db7f6002` + this commit. **Released: v3.57.0** (tag `19330ebd`; HEAD is 5 commits past it: the hifz jump/reciter work, unreleased, and HANDOVER notes). Verified now: `flutter analyze lib test` → No issues; `flutter test` → **529 pass**; hosted, range-requested with a User-Agent, all **206**: `hadith/hadith.zip` 22,235,941 B, `books/text/al_fiqh_al_manhaji_hajj.json` 39,862 B, `books/text/tafsir_al_tabari.json` 7,551,247 B, `books/text/lisan_al_arab.json` 11,825,627 B, `mushaf/madinah_qc/001.png` 40,445 B, `asr/whisper-tiny-ar-quran/ggml-model.bin` 77,691,713 B. Measured: 7 locales × **1,769** keys (identical); library **239** books; `hadith.db` 109,731,840 B / 67,153 rows; pubspec `3.57.0+59`; v3.57.0 APK 264,382,900 B. Mushaf edition count NOT re-measured this session. **Owner confirmed on his phone: tasmee works (v3.57.0).** Open work: the «NEXT CHANGES» list below, item 0 first. |
| **RELEASED 2026-09-23 (v3.57.0)** | tag `19330ebd` = HEAD at publish; asset `RafeeqAlDarb-v3.57.0.apk` 264,382,900 B, download 200 verified; v3.56.0 + tag deleted, v3.51.0 kept. Library **239** (phase 2 complete). Hajj guide now reads الفقه المنهجي (Shamela 6369, vol. 2 pp. 111-188) with al-Jaziri under each step and «المواقيت اليوم» from haj.gov.sa; no citations under the text (owner). Tasmee: the generic-model bug fixed; «حفظي» (any range); continuous recitation asks for the reciter. **Not seen on a real phone:** tasmee with the Quran model; opening the big encyclopaedias (al-Tabari 16,699 pp). |
| **STATE 2026-09-23 (handover)** | `master` pushed. **No release since v3.55.0** (tag = `bb476739` ≠ `HEAD`); `pubspec` still `3.55.0+57`. A signed release APK was built at 02:00 and **not published** — the owner said not to release. Verified now: `flutter analyze lib test` → No issues; `flutter test` → **507 pass**; hosted: `books/text/ilam_al_muwaqqiin.json` 200/1,487,925 B, `hadith/hadith.zip` 200/22,235,941 B, `mushaf/madinah_qc/001.png` 200/40,445 B, `asr/whisper-tiny-ar-quran/ggml-model.bin` 200/77,691,713 B. Library **230 books** (phase 2's first, إعلام الموقعين, is in). **Tasmee ran on the owner's Xiaomi — but NOT on the Quran-tuned model** (corrected later the same day): the model was saved as `ggml-model.bin`, whisper_flutter_new looks for `ggml-tiny.bin` and silently downloaded whisper.cpp's GENERIC tiny model instead. Fixed and released in v3.57.0 (name + sha256 check + automatic repair); not yet re-tried on a phone. **Five phase-2 crawls are still running** as detached processes (الطبري، القرطبي، فتح الباري، المغني، المجموع) — they keep going after this session ends, and their files land in `scripts/book_text_build/`. |
...
| **On the owner's phone** | he installs from GitHub Releases; once he updates he has **v3.53.0**, and v3.51.0 is still downloadable beside it. **3.53.0 has NOT been on a real phone yet** — only emulator-5554 |
...
A full round of his own findings, walked screen by screen on his phone.

* **The dedication.** «أُهدي هذا العمل إلى زوجتي ورفيقة دربي» above the du'a
  in «عن التطبيق», in the Quran face, and the du'a now opens «كما أسألكم».
* **The store screenshots are in «شرح ميزات التطبيق»** — asked for three
  times. `FeatureGalleryScreen`, eight shots, captions from keys the app
  already had. The size was why it kept not happening: 6.65 MB of PNG is
  0.42 MB at 540×960 JPEG (`scripts/build_tutorial_shots.py`).
* **`SciencesPackScreen`** lists what علوم القرآن holds, every number
  COUNTED from the database rather than written down.
* **التنزيلات**: every row opens something, back returns to it, the voices
  row goes to its setting, the duplicate mushafs row is gone, and the
  mushaf tile's line is readable (it was `colorScheme.outline`, the
  hairline grey).
* **A sixth permission**, the adhan's full screen — Android 14 withholds
  `USE_FULL_SCREEN_INTENT` and without it the adhan cannot take the lock
  screen.
* **Three raw edges**: التنزيلات was two cards with one title, «الأدوات»
  printed `tutorial.title` because that key does not exist (trap #8, live on
  his phone), and the language chooser appeared on two consecutive screens.
* **Library sub-tabs** stopped clipping their own labels.

SEEN on emulator-5554: the whole first run, the gallery, and المزيد.

## WHAT v3.50.0 SHIPPED (2026-09-21)

One fix on top of 3.49.0, his: «اللغة اتكررت في شاشة الاذونات وشاشة تحميل
المصحف». Moving the language chooser onto the permissions page left the old
copy on «اختر مصحفك» right behind it, so the same seven chips appeared on
two consecutive screens. The one on the mushaf page is gone; the permissions
page owns it, because that is the page that has to be readable first.

SEEN on emulator-5554, fresh install: «Choose your Mushaf» now carries the
two downloads and nothing else.

**The restore point is v3.50.0** — `known-good/v3.50.0`, `backup-2026-09-21b`
and the APK. Earlier restore refs are left exactly where they are.

## WHAT v3.49.0 SHIPPED (2026-09-21)
...
<!-- WIP:START -->
**2026-09-26 22:52 — IN PROGRESS — resume here**

Dorar check everywhere IN CODE: hadith books, daily hadith, hadeethenc, adhkar, book text selection; 615 pass, analyze clean; NOT built
