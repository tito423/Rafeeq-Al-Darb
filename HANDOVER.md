...
<!-- WIP:START -->
**2026-09-06 14:54 — IN PROGRESS — resume here**

P3-53 parts 1-3+5. Part 1 (lock-screen adhan): removed the 'video shows only while screen on (Android limitation)' note from Adhan settings + the video_note key from all 6 locales; added programmatic setShowWhenLocked/setTurnScreenOn (API27+) with pre-27 window-flag fallback in MainActivity.onCreate as belt-and-braces alongside the manifest attrs, so the full-screen-intent adhan draws over the lock screen without biometric unlock; scoped WakelockPlus to the adhan player (enable in initState, disable in dispose) so ordinary reading holds no wakelock. Part 2 (download hang): DownloadManager is now a FIFO queue - maxConcurrent=1, tasks enter as queued and _pump promotes one at a time; resume re-queues instead of racing; replaced the per-file flood of progress notifications with ONE aggregated ongoing notification (current file + % + how many waiting), cleared when the queue drains; streaming-to-RAF (no whole-file RAM buffering) was already in place. Part 3 (perf): extracted the adhan live clock into its own self-timing _LiveClock widget so the per-second tick no longer rebuilds the video+subtitle Stack; RepaintBoundary around the video surface. Part 5 (luxury cover): new QuranBookCoverThumbnail - leather board per edition (emerald Hafs, plum Shubah, bronze Duri, wine Qalun, navy Warsh), gold ornamental double frame + corner diamonds, central gold medallion with the riwayah name in AmiriQuran, hanging red reading ribbon, 3D shadow; replaced the Fatiha-page thumbnail in both the edition picker and the downloads tile; deleted the now-orphaned mushaf_first_page_preview.dart. flutter analyze clean, tests 21/21. Live-verified on emulator-5554: luxury covers render correctly in the Choose-a-mushaf sheet; splash video plays; home live clock + prayer card healthy. Part 4 (5 editions): app already ships 5 working editions (Hafs/Shubah/Duri/Qalun/Warsh, all HTTP 200 at the pinned quran-svg source) - 3 of the owner's requested 5 (Hafs/Warsh/Qalun) already present; Tajweed-colored and Shamarly need a real per-page image source (none found free/verified - archive.org only has PDF/JP2 book scans), pending owner input.
