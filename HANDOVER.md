...
<!-- WIP:START -->
**2026-09-09 14:58 — IN PROGRESS — resume here**

checkpoint(wip): The recitation hang after a long spell in the background - the owner's «التلاوة بتهنج ... ولما بحاول اضغط اشغلها مفيش حاجة بتحصل». Cause, from reading just_audio_background 0.0.1-beta.17 rather than guessing: Android tears down the audio_service foreground service after a long background stretch and the platform player goes with it, while this isolate lives on believing a recitation is still running. play() on a released player returns silently, setAudioSource throws - and the old catch arm called stopContinuous() and gave up. So every press did nothing: the transport play button no-opped, and the reader toolbar button saw active==true and merely cleared a run that was not running. Four changes. (1) The AudioPlayer is no longer final: the package clears its _playerId in disposePlayer (just_audio_background.dart:143, read in the pub cache), so a NEW player after a disposed one is legal - only a second simultaneous one is refused. _recreatePlayer() does that, and _loadSurahIntoPlayer now makes two attempts, the second on a fresh player with freshly built AudioSources (the same objects must not be handed to a new player). (2) playerState/isPlayingStream are re-broadcast by the service through its own controller, so a StreamBuilder that subscribed before a swap keeps getting events instead of listening to a dead player. (3) ContinuousRecitation gains `stalled`, set on app resume when the state says active but the player holds nothing; the transport then shows PLAY and says «توقّفت التلاوة — اضغط للاستئناف», continuousPauseResume reloads the current verse instead of asking a dead player to play, and both reader toggles resume rather than stop. The resume hook is an AppLifecycleListener owned by the service, so it covers the mushaf reader and the Sunan Suwar reader alike. (4) A load that fails even on a fresh player is now REPORTED - continuousError drives a SnackBar - instead of ending in a silence indistinguishable from an ignored press. Three keys added across all 7 locales. analyze clean, 40/40. Device verification still pending; this is the change that most needs it
