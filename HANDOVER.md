...
<!-- WIP:START -->
**2026-09-12 11:06 — IN PROGRESS — resume here**

batch 11 stage 8 VERIFIED on emulator-5554, and two stale strings caught by looking at the screen. The صوت|فيديو card is gone from adhan settings - confirmed by opening it. The Fajr test adhan, fired through the REAL alarm pipeline (exact alarm, receiver, foreground service, lock-screen takeover), came up on AdhanScene's fajr sky and it is plainly a different sky from dhuhr's: a star field overhead, a crescent low and to the right with its halo, the cold blue horizon band with warmth under it, and the mosque's gold rim light on the RIGHT edge - the side the crescent is actually on, which is the lighting-follows-the-source logic working. That test also proves the thing this removal actually risked: the whole alarm chain still fires with videoPath gone from Dart AND from Kotlin AdhanSpec.kt. Two strings still said فيديو after the removal and were only findable by reading the screen: «معاينة الأذان» promised «لتجربة الفيديو والصوت», and the downloads hub's adhan row was literally «الأذان (فيديو)». Both rewritten in all seven locales - the preview now says الصوت والمشهد, and the row is just «الأذان» because it still legitimately holds whatever a phone downloaded until purgeAdhanVideos has run. analyze clean, 194 tests pass.
