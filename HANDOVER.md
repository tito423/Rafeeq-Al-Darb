...
<!-- WIP:START -->
**2026-09-03 21:31 — IN PROGRESS — resume here**

PHASE3 ROUND 2 consolidated: owner resent his original feedback message verbatim (asked to verify item-by-item against status - cross-referenced, not re-logged) plus a genuinely new 'ثانيا' list of 18 more items (P3-20 through P3-37), all written up in PHASE3.md with a fresh status table. Analyzed an attached video of an EARLIER WORKING BUILD of this same app (design_refs/old_app_video.mp4 + 14 extracted frames in design_refs/old_app_frames/) using opencv (no ffmpeg available, installed opencv-python-headless) - this is now the frame-verified exact target for P3-4/P3-22's remaining animated prayer card work (live HH:MM:SS clock, real countdown pill, location line, 4 colored prayer chips with a القادمة badge) and P3-20's splash screen restyle. IMPORTANT FINDING flagged prominently in PHASE3.md: the video's mushaf-choice onboarding screen showed 17 editions using the EXACT internal names (مصحف 12 سطر, مصحف التهجد وقيام الليل, أوردو 12/13 سطر, ورش الأصبهاني) HANDOVER.md §5.1 already identified as QuranFlash-derived and purged once - strong evidence that screen's cover thumbnails came from the same banned source. Flagged this clearly: will NOT recreate that 17-edition catalog or its thumbnails, will only build onboarding around this project's real 5 legitimately-sourced editions. Several new items explicitly noted as blocked on images still owed (icon round 2, book-reader Shamela-app reference) or as likely-stale (Azkar/Tasbeeh 'still old' - probably written before he saw the batch-2 APK that already fixed both). No code changed this step - pure consolidation/documentation as explicitly requested ('راجع كل اللي طلبته وقسمه').
