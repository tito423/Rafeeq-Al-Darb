...
<!-- WIP:START -->
**2026-09-08 15:08 — IN PROGRESS — resume here**

Publish v3.1.0, clean the release history down to two, and correct both remaining tags. Released v3.1.0 as Latest with the APK built from 7c6c47f, then deleted the other 31 releases and their tags at the owner's request, keeping only v3.1.0 and v2.1.21 as a known-good rollback so the release list stops being confusing. Caught while verifying: the release had been cut with --target master before master was ever pushed, so its tag landed on the previous head (40b3093) rather than the code the uploaded APK was actually built from. Pushed the five local commits (fast-forward 40b3093..7c6c47f) and retargeted v3.1.0 onto 7c6c47f, so the tag, the source and the published binary now agree. Also pushed the corrected v2.1.21 tag, which had pointed at the repo's initial commit ce22947 like every other tag did, onto its real commit 0686854 - the mapping evidenced by the release timestamp and by its title matching that commit's subject verbatim. Added RafeeqAlDarb-*.apk to .gitignore: the staged upload artifact belongs on the release, not committed as a 240 MB binary per version. Emulator-verified before publishing that the running header no longer names a wrong surah on Shamarly's last page - it now shows the page number alone.
