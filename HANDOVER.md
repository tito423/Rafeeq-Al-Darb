...
<!-- WIP:START -->
**2026-09-17 18:33 — IN PROGRESS — resume here**

checkpoint(wip): CONTENT-LICENSES.md now carries the correction rather than the claim. A box above the original table says plainly that every audit re-run returns 0 on that page describes the MORNING run and that the run under-counted, with the before and after side by side: MODERN_EDITOR_APPARATUS_PRESENT 0 -> 5 and a bucket that did not exist, APPARATUS_BUT_NO_EDITOR_NAMED, at 7. Twelve books, not zero. Nothing above the box was deleted - the record of what was believed and on what evidence is the point of the file, and a provenance document that quietly edits its own past claims is worth less than one that shows them being corrected. A new section sets out الإيضاح and the Hajj screen in full: the edition card that says وعليه and names عبد الفتاح حسين, the 280 of 1474 paragraphs (19.0 percent) the screen was attributing to an-Nawawi, why filtering alone would have made it worse because hajj_screen slices by paragraph index, the twelve closing boundaries that turned out to be his notes - one of them literally his signature, قال جامع هذا التعليق المسمى بالإفصاح - and that every boundary snapped inwards and never outwards. It also records the 78.6 percent measurement of mine that was wrong and never left the machine, because the next person to measure this will hit the same trap: the stripper edits markers INSIDE surviving paragraphs, so exact-string comparison counts intact text as deleted. Four books still building
