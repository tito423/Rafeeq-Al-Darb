...
<!-- WIP:START -->
**2026-09-09 06:56 — IN PROGRESS — resume here**

Ayah highlighting + tap-to-sciences on the Tajweed mushaf, by fitting the Hafs polygon layer onto it instead of building a second one. Established by measurement, not assumption: line positions on 30 scans and on the Hafs polygons BOTH collapse to exactly 15 clusters with matching per-cluster counts, so the printing really does set the Madinah grid. Fitted x'=0.980009x+0.013937, y'=0.974249y+0.017192; the two illuminated opening pages carry their own frame, their own text block AND their own pixel size, so they get their own fits. Every one of the 604 pages' real dimensions was read from its JPEG header - exactly two groups, 602 at 861x1317 and pages 1-2 at 901x1476, no exceptions. Verified two ways: 841 real polygon rings across 86 pages land a median 3.9px from their printed line on an 84px pitch (95th pct 11.9px), and the mapped polygons were RENDERED over pages 1,2,50,200,400,584,604 and looked at - including 604, which sets three surahs and three basmalahs that the polygons correctly skip. The three outliers over half a line were my line detector matching a surah header, confirmed by looking at 584. AyahCoordsRepository is now keyed by asset path, not edition id, so both editions share one parse of the 0.7MB layer; the raster page is laid out in an AspectRatio of its own measured shape because BoxFit.contain alone leaves the drawn rect unknown and the highlight would float free. shamarly/madinah_gold/indopak_tajweed still ship no highlight - they typeset their own way and no affine can fix that. analyze clean, 30/30 tests (5 new, pinned to measured pixel positions)
