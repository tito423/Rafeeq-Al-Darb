...
<!-- WIP:START -->
**2026-09-12 12:30 — IN PROGRESS — resume here**

batch 11 stage 9: the channels the owner asked for were INVISIBLE, and opening the tab is what found it. There were two channel lists - islamic_channels.dart (verified UC ids, mirrored avatars) feeding IslamicChannelsScreen, and a private list inside library_screen.dart feeding the «قنوات دعوية» tab the reader actually sees - and NOTHING in the app routed to IslamicChannelsScreen. So all six channels added earlier in this batch went into a list no one could open, and About counted them into «١٣ قناة», a figure about a screen that did not exist for the reader. analyze and 194 tests said nothing; §1.3 found it. Merging the two lists meant verifying the nine dawah entries, which found THREE dead links already shipping: @Dr.AhmedAlarabi, @MakanyChannel and @waikishow all 404. أحمد العربي recovered at UCyc-c-r8RTLNsp1Uv2DNrSg and وعي at @waey_official (note @waei resolves but to a channel called 'Will' - a handle that merely loads is not evidence); قناة مكاني was NOT recovered and is deleted rather than kept as a card that opens nothing. Twenty-two channels in one verified list now, rendered by the tab, with the orphan screen and the private list deleted. Avatars: downloaded all nine and LOOKED at them - five are YouTube's generated letter tile (A for Amgad and Ayman, H for Hassan and Haytham, f for fahem), confirmed not a scraping error by checking og:image against the avatar block in ytInitialData and getting the same URL both ways. Those five are not mirrored at all and carry hasPhoto:false so the card draws the app's own mark; declining to re-host a placeholder is not inventing one. test/channels_reachable_test.dart guards all three properties. DEVICE: all 22 seen rendering in the tab, real photos and icon fallbacks both. analyze clean, 197 tests pass.
