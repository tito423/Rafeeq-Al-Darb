...
<!-- WIP:START -->
**2026-09-17 19:54 — IN PROGRESS — resume here**

checkpoint(wip): the zoom was heavy for a structural reason, not a tuning one. The image mushaf is an InteractiveViewer living INSIDE a horizontally-swiping PageView, and both wanted the same horizontal drag. Flutter settles that in the gesture arena, which cost two things: at rest every swipe went to the arena before the PageView could take it, so the page turn began late - that is the تحسها تقيلة - and zoomed in, a pan to look at the right-hand side of the page TURNED the page instead, because the PageView is the parent and wins. Tuning thresholds would not have fixed either; the two had to stop competing. So the viewer now takes pans only while it is actually zoomed (panEnabled: _zoomed), which hands the horizontal drag to the PageView outright at rest, and the PageView freezes while zoomed so the pan is the viewer's. The zoom flag is read off the matrix with a tolerance rather than scale > 1, because the controller lands on 1.0000000002 after a pinch-out and a page that thinks it is still zoomed never gives the swipe back. It resets on a page turn and on dispose - a PageView left frozen because a disposed page said zoomed is a mushaf that cannot be turned at all, so that reset is inside a post-frame callback rather than skipped. quran_screen.dart is grandfathered at 1047 and is exactly 1047: the physics line and the import were paid for by tightening two comments that restated what the code already said. No double-tap-to-zoom was added, deliberately - the single tap now opens the floating controls, and a double-tap recogniser would delay every one of those taps by the double-tap timeout, making the thing he called heavy measurably heavier. analyze clean, 342 tests, one red waiting on al_muwafaqat. NOT YET SEEN ON THE DEVICE - next session verify a pinch and a pan on the image mushaf before trusting this
