...
<!-- WIP:START -->
**2026-09-08 19:57 — IN PROGRESS — resume here**

Add Home clock settings with an RGB analogue face, and replace the content claim in About with a sources claim. About: dropped 'real content' from all seven locales in favour of 'built on trusted sources that are freely available to developers', plus a line saying every source is named so the reader can verify it. The first was a claim about the content itself; the second is a claim about provenance, which is the one the Sources screen can actually back up line by line - the owner asked for this specifically on scholarly-honesty grounds. Home clock: new ClockSettings store with three persisted options, surfaced in a Home clock section in More - style (digital or RGB analogue), 12-hour time, and show-seconds. 12-hour is the default because the prayer chips directly under the clock already read as 5:12 PM, so the 24-hour clock above them was the odd one out; AM/PM is localised (Arabic gets ص/م) and Arabic keeps its Arabic-Indic digits. The digital face uses tabular figures so its width stops jittering every second. New RgbAnalogClock paints a full spectrum rim, hour ticks, three hands and a hub. Its colour is driven by the seconds hand's own angle rather than a separate animation controller, so sweep and hue advance together by construction and the widget needs no ticker of its own - the card already rebuilds once a second. The three hands sit at different points on the hue wheel so they stay legible against each other instead of all being one colour at once, and each has a glow and a counterweight past the centre like a real hand. The card itself gained a teal cast shadow, softer corners and clipping so it lifts off the page. Verified on the emulator in both styles, including two captures four seconds apart showing the spectrum rotating with the seconds hand. Prayer times, next-prayer pill, countdown and location all still correct. flutter analyze clean, flutter test 25/25.
