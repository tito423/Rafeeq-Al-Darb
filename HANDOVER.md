...
<!-- WIP:START -->
**2026-09-09 14:46 — IN PROGRESS — resume here**

checkpoint(wip): Found the continuous-recitation bug by reading, and proved it with a test that fails on the old code. quran_screen.dart handed startContinuous its `edition:` argument a MUSHAF printing id (`edition?.id ?? 'hafs_kfqc'`) where a RECITER id belongs - both are String, so analyze saw nothing. Every verse then resolved to cdn.islamic.network/quran/audio/128/hafs_kfqc/<n>.mp3, which 404s; setAudioSource threw, the catch arm called stopContinuous(), and picking a verse mid-recitation STOPPED the recitation instead of moving it. That is the owner's report word for word. The Sunan Suwar screen had it right all along - one copy diverged. test/recitation_edition_test.dart scans every startContinuous call site in lib/ against the real editions.json ids; reintroducing 'hafs_kfqc' makes it fail naming quran_screen.dart:799. Second half of the same complaint: the sciences sheet's play/stop toggle read the SHARED player, so mid-recitation it rendered as STOP and stopQueue() killed the player while continuous.value stayed active - a zombie transport bar over silence. It is now recitation-aware («اقرأ من هنا»), jumps the recitation to the tapped verse via the new jumpContinuousTo() and closes the sheet; and play() now supersedes continuous properly instead of leaving a stale completion listener that fired _advanceToNextSurah on the single ayah's end. Also: default theme is now LIGHT on a fresh install (an existing dark install is migrated as dark, spelled out rather than left to the default arm); the splash video gains a sound on/off setting (volume, not a skipped video, default on so no existing install changes behaviour) in all 7 locales; and the startup permission prompts now wait 900ms - past the 300ms MaterialPageRoute transition - so the OS dialog can no longer come up over a splash that is still fading out. analyze clean, 40/40 tests (37 + 3 new). NOT yet run on the device
