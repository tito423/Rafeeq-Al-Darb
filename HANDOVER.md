...
<!-- WIP:START -->
**2026-09-17 15:04 — IN PROGRESS — resume here**

the fix now actually REACHES a device, which is the thing that would have made all of today pointless. 47 books were re-uploaded over the same R2 keys this morning and nothing told a phone about it: book_meta carries no version - hadith.db has AppConfig.hadithDbVersion for exactly this and books had no equivalent - so a reader holding a pre-filter copy would have kept the editor's apparatus for ever while the card said «تمّ التنزيل», which was true and useless. isBookDownloaded now compares the local file's byte length against the catalogue's sizeBytes, a number measured from the bucket on every upload and never typed by hand. A book whose entry records 0 is left alone, because unknown is not stale. The check is only as good as the catalogue so the catalogue was checked FIRST: verify_catalog_sizes.py over all 248 found two that had drifted - mawaiz_ibn_al_jawzi_al_yaqutah by 7 bytes and bustan_al_arifin by 14 - both corrected from the bucket's own readback, and it now reports 0 mismatches over 248. That script's first version reported five FALSE mismatches because R2 answers a plain HEAD for some objects with no Content-Length at all; it uses a one-byte range GET and reads Content-Range now. Proven on the emulator in both directions, not argued: with the pre-filter al-Adhkar in place (390,635 bytes against a catalogued 320,674) the book DISAPPEARS from «مكتبتي», and with the current file back it returns with its Open button. And the run caught something I had not planted - al_idah_fi_manasik_al_hajj_wal_umrah stayed hidden because the copy a previous session pushed by hand is 445,178 bytes where the bucket and the catalogue both say 445,183. A real drift, found by the check on its first outing. analyze clean, 294 tests
