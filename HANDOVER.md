...
<!-- WIP:START -->
**2026-09-17 15:32 — IN PROGRESS — resume here**

the ruqyah duas, located in an-Nawawi and curated - and one of the six is honestly missing. ruqyah_catalog.dart addresses six duas by Hisn al-Muslim row id, and rebuilding azkar_items from a different book would have left those ids pointing at whatever landed on that row: the comment «a mis-typed id shows up as a missing dua, not a wrong one» stops being true the moment the table is rebuilt. So each was searched for BY TEXT across all 338 chapters. Five are there: «لا بأس طهور إن شاء الله» (al-Bukhari), «أسأل الله العظيم رب العرش العظيم أن يشفيك» (al-Tirmidhi, hasan), «بسم الله ثلاثا... أعوذ بعزة الله وقدرته من شر ما أجد وأحاذر» (Muslim), «أعوذ بكلمات الله التامات من شر ما خلق» (Malik's Muwatta), and «أعوذ بكلمات الله التامة من غضبه وشر عباده ومن همزات الشياطين» (al-Tirmidhi, hasan). The sixth - «التي لا يجاوزهن بر ولا فاجر» - returns ZERO hits in this edition, so it is dropped rather than reconstructed, which is the screen's own stated principle. They carry explicit ids 1001-1005 so the screen can address them for ever instead of depending on row order. And a real defect in the tool, found by READING the output rather than trusting errors=0: hadith ٣٨٨'s takhrij came back as the whole hadith, because «صحيح مسلم» occurs twice in it and the slice took everything between the two. slice_span now treats identical from/to anchors as «this phrase and nothing else», and the rebuild reported 0 errors, which means no other entry's checksum moved - only the one I meant to change
