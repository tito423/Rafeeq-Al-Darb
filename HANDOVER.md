...
<!-- WIP:START -->
**2026-09-12 04:20 — IN PROGRESS — resume here**

batch 11 stage 3 (code only, not yet on a device): two more channels verified and added (mabrouk_zidelkhir UCCLucFt_j51ToCFyiE8qMVA, rateb_alnabulsi UC7naRnmAOTwDPu738W2SljQ) with avatars mirrored to R2 and looked at - for النابلسي the encyclopaedia's own channel was taken over the two republishing channels. About screen rebuilt so every figure on it is COUNTED at runtime rather than typed: it had said seven mushafs when six ship and had named four printings that are not in editions.json (الشمرلي، الهندي، ورش، قالون) while omitting قطر والكويت والليلية; ten muezzins against twelve in adhans.json; four calculation methods against twenty; over forty thousand hadiths against 67,153 rows; and a version badge of 3.1.0 against pubspec 3.19.0 - eighteen releases of drift. Counts now come from mushafEditionsProvider, quranTranslationCatalogProvider, recitersProvider, adhanCatalogProvider, kPrayerCalculationMethods, libraryBookCatalog, islamicChannels, kSupportedLocales and three COUNT(*)s on the open hadith.db (HadithRepository.counts added); a row whose catalogue has not loaded shows the sentence without its number rather than a guess. test/about_version_test.dart pins the one figure that cannot be counted, the version, against pubspec.yaml. The owner's du'a card added under the hero - AmiriQuran on the patterned panel with a breathing lamp. Sources screen now credits each scanned printing separately with its own item and the rights position that item or volume actually states (tajweed, gold CC BY-NC-ND, qatar CC BY-NC-SA 3.0, kuwait ministry no-notice, madinah night King Fahd no-notice) instead of one archive.org line covering five different books, plus a YouTube row for the mirrored channel avatars. analyze clean, 192 tests pass.
