...
<!-- WIP:START -->
**2026-09-06 03:50 — IN PROGRESS — resume here**

P3-49b: put back the owner's AI-generated splash video with the Gemini watermark removed. Recovered his splash_intro.mp4 from git history (the Gemini/Veo-made رفيق الدرب intro I had deleted when removing the splash), located the persistent Gemini sparkle watermark near the bottom-center (720x1280), and cleanly removed it with ffmpeg's delogo filter (interpolated from the surrounding sky) - verified frame-by-frame that the watermark is gone and the logo/title/tagline are intact. Restored the splash feature files from history (splash_screen/splash_lattice/splash_video_provider), re-wired rafeeq_app.dart home->SplashScreen, re-added assets/branding to pubspec, re-added the Settings 'splash video' toggle + its 6-locale keys, and defaulted the toggle ON so the video actually plays (not just first run). flutter analyze/test clean (21/21); confirmed the cleaned 2.68MB video is bundled in the APK. Emulator showed the lattice fallback (its video decoder is unreliable) - the asset is the same h264 format as the original that played fine on the owner's real device.
