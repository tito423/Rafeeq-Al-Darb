...
<!-- WIP:START -->
**2026-09-09 16:18 — IN PROGRESS — resume here**

Localisation, three asks - two delivered whole, one delivered as far as honest sourcing allows, all verified on emulator-5554 in Spanish. (1) The Quran translation now FOLLOWS the app language. quran_sciences.db already carries a complete translation - all 6,236 ayahs - in exactly the six non-Arabic locales the app ships (en, fr, ur, es, ru, pt), so every UI language has a real translation behind it and nothing is guessed. A fresh Spanish install now opens the Traduccion tab already on Espanol, Julio Cortes, rather than on English. It follows a CHANGE of app language rather than every rebuild, so an explicit pick still survives. (2) Mushaf and reciter names in the app language. A fresh Spanish install used to list every printing under its ARABIC title on an otherwise Spanish screen; it now reads Mushaf de Medina, Mushaf de Tajwid en color, Mushaf del Estado de Kuwait, and the reciter picker says Minshawy (Mujawwad) instead of the Arabic. Nine editions gained a title and a riwayah line in es, fr, pt, ru, ur - these are descriptive TITLES of printings, ordinary translation. A reciter name is a person's name, so it is transliterated, not translated, and the choice is by SCRIPT not by language: Arabic script for ar and ur, which share the alphabet, and the catalogue's own Latin transliteration for the rest. A test pins all seven locales for all nine editions and fails if any falls back to Arabic. (3) The hadith translation under the hadith - on the daily CARD as well as in the book, which is what was missing. Here is the honest part, measured not assumed: hadith.db carries an English rendering for 36,148 of the 67,153 from sunnah.com's published translations, named translators. There is NO Spanish or Portuguese rendering of these collections in any redistributable source at all; the French, Urdu and Russian sets that circulate publicly - fawazahmed0/hadith-api, public domain - name no translator, and the French Bukhari I sampled is plainly a translation of the ENGLISH rather than of the Arabic. Shipping that as the French translation of Sahih al-Bukhari is the unattributed religious content this project does not ship, so it is not shipped and the owner gets told instead. What IS shipped is the English, and now always LABELLED: a Spanish reader sees "Traduccion al ingles - sunnah.com" above it instead of being left to work out that the paragraph is English; and Musnad Ahmad's 27,584 and al-Darimi's 3,406, which have no translation in any language, now say so instead of showing nothing. Tafsir and i'rab stay Arabic because every tafsir in the DB is Arabic and the grammar labels are Arabic grammatical terms - no translated edition of either is in the app to switch to. analyze clean, 42/42
