...
<!-- WIP:START -->
**2026-09-12 12:54 — IN PROGRESS — resume here**

batch 11 stage 11: the download-notification routing did not work, and finding out cost two invalid measurements before a valid one. First attempt: tapped the shade row and the app came up on Home. Assumed the cause was launchMode and switched MainActivity from singleTop to singleTask, which the downloader plugin's docs ask for - no change. Read the plugin's Android source and found that a GROUP notification's tap intent is built with an empty task string, addTapIntent(taskWorker, '', ...), and BDPlugin.handleIntent skips it, so taskNotificationTapCallback can never fire for a grouped notification - and this app groups all three download kinds on purpose. Wrote a native reader for the plugin's intent. Still Home. Then logcat during the tap: act=android.intent.action.MAIN cat=[LAUNCHER] - so the row I had been tapping was ANDROID'S OWN auto-bundle summary, not the download notification at all. Expanded the group and tapped the real «تحميل ملفات الرقية الصوتية» row: still MAIN/LAUNCHER, which finally proves the plugin's grouped notification carries no routing intent, whatever the manifest says. So routing now rides on the app's OWN notifications, which it fully controls: DownloadForegroundService puts the destination on its launcher intent (a distinct requestCode per destination, because PendingIntent equality ignores extras) and MainActivity reads it back, delivering straight to Dart when it is listening and holding it for takePending on a cold start. That covers the mushaf download the owner named by name. The three background_downloader groups are NOT covered and that is a package limitation, recorded as such. analyze clean. NOT YET RE-TESTED ON THE DEVICE.
