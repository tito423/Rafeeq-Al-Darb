...
<!-- WIP:START -->
**2026-09-06 13:02 — IN PROGRESS — resume here**

P3-51: mushaf layout + badge-centering fixes from the owner's screenshot. (1) The page-number badge no longer floats over the ayah text - in normal mode it now lives in its own dedicated info bar between the text card and the scrollbar (vertical order: text area / page-number bar / scrollbar / app bottom-nav), built with min-size flex so nothing overlaps on any screen size. It's only floated (bottom-center, over the already-reserved 56px strip) in full-screen mode where there is no bottom bar. Removed the page number from _PersistentPageOverlay (now just the surah/juz top running header). (2) Uniform text-centering rule for Arabic badges: page number, surah-name badge, juz badge and the surah-banner cartouche now use alignment center + StrutStyle(forceStrutHeight, height 1.0, leading 0) + TextHeightBehavior trimming first-ascent/last-descent, so the AmiriQuran glyphs (with their tashkeel headroom) land in the true geometric centre instead of drifting off the baseline. New _PageNumberBadge (a properly-centred circle) used in both the bottom bar and the full-screen float. flutter analyze/test clean (21/21); live-verified on emulator: page number sits in its own bar below the text with no overlap, and all badges are centred.
