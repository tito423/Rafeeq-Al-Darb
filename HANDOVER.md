...
<!-- WIP:START -->
**2026-09-08 18:54 — IN PROGRESS — resume here**

State every hadith's takhrij explicitly, and give the Hadith tab two sections. Takhrij: correcting my own earlier claim first - I said the app was showing 'the grade is not stated' on a Bukhari hadith, and it was not; grade_unstated and sahihayn_badge were both dead keys no screen referenced, and the detail screen simply rendered nothing for Bukhari and Muslim. The real problem is that saying nothing is itself ambiguous - a reader cannot tell an ungraded hadith from one whose grading the app forgot to show. Every hadith now carries a takhrij line. Bukhari and Muslim get 'Sahih - reported in Sahih al-Bukhari/Muslim', which is a statement of which collection it is in rather than a grade the app invented: what the two of them included is authentic by the collections' own criteria and by consensus, so the collection is the grading and 14,736 hadiths stop looking unverified. The four Sunan show the source's own grade with the grader named (al-Albani 4,898, Darussalam 13,149). Anything the source does not grade now says so outright instead of rendering blank. Measured coverage, for the record: 18,047 of 40,943 carry a source grading, all of it in the four Sunan at 90-96 percent each; Musnad Ahmad, Muwatta Malik and Sunan al-Darimi have none because the build script found no redistributable graded source for them, which is recorded in its own docstring. Nothing was inferred to fill that. Hadith tab: the catalog's 12 hadith-category books were only reachable through Books > Categories > Hadith, which is not where anyone looks for them. The tab now has a labelled 'Nine Collections' section over the structured corpus and a 'Hadith texts' section over those books, each with its own download button and opening in the text reader. They are kept as two sections rather than one merged list because they are genuinely different - one is browsed by chapter and searched as a corpus, the other is read page by page - and flattening them would put two different interactions under one heading. Also registered seven well-known hadith books in the Shamela pipeline (Bulugh al-Maram, al-Adab al-Mufrad, Sahih al-Adab al-Mufrad, al-Shamail al-Muhammadiyyah, Mishkat al-Masabih, al-Targhib wal-Tarhib, Umdat al-Ahkam). Every Shamela id was checked against its own book card and its pageContent API before being listed, and the edition lines are copied from those cards rather than written from memory. Umdat al-Ahkam already builds clean: 448 pages, 120 chapters, print numbering verified monotonic and matching the printed edition. flutter analyze clean, flutter test 25/25.
