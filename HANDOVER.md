...
<!-- WIP:START -->
**2026-09-12 05:07 — IN PROGRESS — resume here**

batch 11 stage 6 verified on the device, and CLAUDE.md gains traps 43-46. The immersive-mode fix measured before/after on emulator-5554 with dumpsys: statusBars visible=false became visible=true on the library search screen, and opening the keyboard there went from 43 frames rendered with a 101ms median to 10 frames with a 32ms median. The first attempt at that measurement was INVALID and is recorded as such - keyevent 111 had not closed the keyboard, so it measured zero frames of an action that never happened; keyevent 4 closes it, and comparing the bottom third of a screenshot before and after is how that was caught. Traps written up: 43 a process-wide SystemUiMode set from a never-disposed kept-alive tab (with how it was found - screenrecord to a contact sheet, dumpsys window, dumpsys gfxinfo reset/act/read), 44 maybePop inside PopScope canPop:false is a loop and was why the tutorial's skip button did nothing, 45 registerCallbacks diverts the updates stream for status and progress only - read in the package source, not assumed - and 46 Git Bash rewriting /sdcard into C:/Program Files/Git/sdcard before adb sees it.
