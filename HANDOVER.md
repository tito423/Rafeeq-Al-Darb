...
<!-- WIP:START -->
**2026-09-05 08:38 — IN PROGRESS — resume here**

P3-43 item #7 done + live-verified: persistent mushaf running-header overlay (page number bottom-center, surah name top-right, juz name top-left) - new _PersistentPageOverlay sits in a Stack above the PageView so it renders identically in both mushaf modes and stays fully immune to toolbar-visibility/full-screen state (IgnorePointer, lives outside both conditional trees). Surah/juz derived by the same real highest-start-page-at-or-before-current rule _SurahStrip already uses. Deliberately fixed physical left/right corners (not RTL start/end) and always-Arabic text for both badges, matching how the existing surah banner and mushaf_nav_sheets.dart's own juz list already work - a real mushaf's printed running header is part of the page's identity, not app chrome that follows the interface locale. flutter analyze/test clean (15/15). Live-verified on emulator-5554 batch27: overlay renders correctly in normal text-mode reading and, critically, is still visible with all three chrome layers (AppBar, Quran's own bottom bar, AppShell's nav bar) hidden in full-screen mode - confirmed via screenshot with only the badges and page content on screen.
