...
<!-- WIP:START -->
**2026-09-05 14:01 — IN PROGRESS — resume here**

checkpoint(wip): P3-43 item #13 done + live-verified: Azkar hub re-categorized to match the reference's 10-category colored-card layout - 8 of the 10 built as real content-based groupings of the actual 133 Hisn al-Muslim sections (new azkar_categories.dart maps every section by title/content, not bulk-guessed; section 29 'azkar al-sabah wal-masaa' deliberately listed under both Morning and Evening since Hisn al-Muslim never split it into two chapters). The other 2 target categories (Quranic supplications / legal ruqyah) are deliberately NOT rendered rather than force-fit or shown empty: grepped all 134 real section titles for the Arabic words for Quran and ruqyah and got zero matches for both, confirmed via schema that azkar_items has no source-type column to derive one honestly either - inventing a dead-end category card would be exactly the placeholder UI rule 1 forbids. azkar_screen.dart's flat grid replaced with colored gradient category headers + per-category section grids in a situational daily-flow order; 8 new azkar.category_* keys added across all 6 locales. flutter analyze/test clean (21/21, parity holds). Live-verified on emulator-5554: fresh debug build, all 8 categories render with correct real sections underneath, scrolled the full list with zero crashes/overflow, tapped into a real section and confirmed the dhikr text+source still render correctly, adb logcat clean throughout.
