...
<!-- WIP:START -->
**2026-09-08 13:37 — IN PROGRESS — resume here**

Add three real mushaf PRINTINGS and 47 Quran translation languages. Mushaf: the owner clarified he wanted printings (tabaat), not more riwayat, and asked for six more. Searching archive.org rather than only GitHub turned up real per-page sets that earlier sessions had recorded as nonexistent. Shipped three, each verified page-by-page on R2 before being listed: Shamarly (archive.org quran_pages_shamarly, 521 numbered PNGs, 1..521 with none missing, visually confirmed as the real Shamarly typesetting with its blue and orange ornamental header); the fully gold-illuminated Madinah printing (archive.org smartmushaf, 604 JPEGs at 1769x2598, downscaled here to 1200px wide because 800KB a page is 490MB for one edition); and the Indo-Pak Nastaliq colour-coded tajweed printing (Zia-ul-Quran, Lahore/Karachi) rendered out of a scanned PDF with PyMuPDF, after confirming visually that five cover and title pages precede the mushaf so page 1 is PDF index 5, giving 564 pages. New scripts/r2_upload_mushaf_printings.py does the resize/flatten/render; the older script gained the Shamarly source. All six raster editions now verify complete on R2: tajweed 604, warsh 604, qaloon 604, shamarly 521, madinah_gold 604, indopak_tajweed 564. Page counts genuinely differ between printings, which editions.json already models per edition. Cover palette and download-tile colours extended for the three new ids, with medallion overrides naming the printing since all three are Hafs and would otherwise all read hafs. Translations: the owner clarified the 30+ languages ask was for the QURAN TRANSLATION only, not the app UI, which stays on its six locales. Added 47 languages from api.alquran.cloud, the provider already credited in the app, one established translation each (Diyanet for Turkish, Bubenheim for German, Ma Jian for Chinese, and so on), rehosted gzipped to R2 at 16.2MB total by new scripts/r2_upload_quran_translations.py. The six bundled translations stay in quran_sciences.db so offline behaviour never regresses; the other 41 download on demand at 250-550KB each into a new quran_translations.db via QuranTranslationStore, which sniffs the gzip magic number rather than trusting Content-Encoding and rejects any payload that is not the full verse count. Two upstream editions (Korean, Kurdish) have exactly one verse left blank at source, reproducibly, so they ship with their real 6235 and the catalog records the true count rather than hiding the gap. The translation tab now lists every language by its own native name with the download size shown, downloads inline on selection, and lays out RTL languages correctly. flutter analyze clean, flutter test 25/25.
