...
<!-- WIP:START -->
**2026-09-03 20:05 — IN PROGRESS — resume here**

PHASE3 P3-15 library expansion DONE (first batch) + P3-19 dead-code cleanup + P3-7 J4 fix, analyze clean + test 15/15: Added 3 new Shamela-sourced نص-only books matching the owner's original P2-4 author requests (Ibn Taymiyyah's العقيدة الواسطية 72p, al-Hakim al-Tirmidhi's نوادر الأصول 1237p across 4 vols - honesty note about weak-hadith content carried into descriptionAr itself, Ibn Abi al-Dunya's الصمت وآداب اللسان 787p), built via build_book_text.py, uploaded to R2 (not GitHub - matches the P3-17 migration), byte-verified via head_object+curl. LibraryBook's image-PDF fields (downloadUrl/fileName/approxSizeBytes/sourceUrl) made nullable + new hasImage getter, so نص-only books need no fake مصور entry; مصوّر|نص switch now only shows when both exist. Also DONE: مكتبتي split into two headed sections (مصوّر/نصي) instead of one interleaved list. Real dead-code find: MainActivity.kt had a SECOND, entirely unused native prayer-notification implementation (salatuk_notification RemoteViews channel, never called from any Dart code, completely undocumented in HANDOVER's P2-6 history) - removed the handler + orphaned function + unused layout xml. Reviewed prayer_status_notification.dart and the Adhan Stop/Mute wiring closely for the P6/P7 real-device bugs - no code defect found by static review (both match what HANDOVER already documented as emulator-verified), genuinely needs live device/logcat now; DID find one concrete, real, fixable gap along the way - Android 14+ requires a separate manually-granted full-screen-notifications permission beyond the manifest declaration, a real candidate for 'notification shows, full-screen never does', not yet built as a settings card. J4 DONE: selecting an adhan in the picker now auto-plays its preview immediately (RadioGroup onChanged -> _togglePreview forcePlay), no separate play-tap required; reuses the single shared preview player so switching selection correctly stops whatever was previously previewing.
