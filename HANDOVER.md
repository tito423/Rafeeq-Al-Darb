...
<!-- WIP:START -->
**2026-09-13 02:00 — IN PROGRESS — resume here**

the design is now LOCKED BY A TEST, not by a request in a prompt. The owner handed this repo's prompt to Gemini Pro 3.1 once and got the design changed underneath him - «غير في التصميم مع اني خدت البرومبت منك وادّيتهوله» - and he is about to hand the remaining work to another agent, so a sentence asking an agent not to touch the palette is worth nothing. test/design_lock_test.dart pins all 26 AppColors constants by exact value, and CLAUDE.md section 2.2 forbids calling anything done while flutter test is red, so any agent that touches a colour is stopped by its own definition of done. PROVEN to fail on the change it is meant to catch: flipping the gold from 0xFFD4AF37 to 0xFFD4AF38 - one digit - made it report the exact blue channel difference, and the file was restored. It guards against me as much as anyone: a colour is easy to improve in passing. The comment says the only legitimate way to move it is a deliberate edit the owner asked for, in the same commit as the change. 212 tests pass, analyze clean.
