...
<!-- WIP:START -->
**2026-09-13 10:47 — COMPLETE**

NEXT_PROMPT.md rewritten against today's actual state, while waiting for the owner to pick the next item. It now says the two things a fresh session would otherwise get wrong: v3.23.0 is published but the tree has TWO unpublished changes on top of it (the bundled mushaf removed, 324.6 -> 226.7 MB, and 28 books in the catalogue), and the owner has said to hold the release - so the next one is on his word, not automatic, but the session must not die with that unpublished either. Added as item 3 a consequence of removing the bundle that nobody should discover by accident: the DEFAULT mushaf now depends on raw.githubusercontent.com (quranpedia/quran-svg at a pinned commit, probed 200 image/svg+xml on pages 1, 302 and 604) and R2 does not mirror it (404), with the exact git command to restore the 604 SVGs from history and upload them to the bucket. The countdown jump keeps its own section and is still labelled NOT PROVEN, because it never reproduced on the emulator. Also recorded: the owner has not answered the offer to keep the first ~20 mushaf pages bundled so the reader opens with no network.
