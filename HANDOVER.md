...
<!-- WIP:START -->
**2026-09-12 14:01 — IN PROGRESS — resume here**

refactor stage 2: ayah_sciences_sheet.dart was 1,543 lines and twenty classes - a header, three dialogs, the ayah panel, four tabs and the shells they are all built from, every one of them private to that single file and therefore unreachable from anywhere else. It is a 183-line shell now (the sheet, its state and the drag handle) over lib/features/quran/presentation/widgets/ayah_sciences/: sciences_header 374, irab_tab 329, translation_tab 249, sciences_dialogs 206, sciences_common 149, ayah_panel 112, tafseer_tab 106. Eleven types that now cross a file boundary lost their underscore - SciencesHeader, RepeatDialog, NoteDialog, ReciterChip, AyahPanel, AsyncTab, SciencesNotice, SourceBlock and the three tabs - while _GrammarCard and _Chip stayed private because only the irab tab draws them. 143 imports were stripped, by reading the analyzer's own unused_import report rather than by eye. VERIFIED on emulator-5554: long-pressing ayah 2:7 in the cards layout opens the sheet, and all three tabs render - التفسير serves التفسير الميسّر from the bundled db, الترجمة serves Saheeh International with its source credited, and الإعراب draws the word cards with root, case and gloss. The header shows 2:7, تلاوة الآية and the reciter chip under the title. analyze clean, 199 tests pass.
