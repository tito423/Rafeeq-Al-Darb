...
<!-- WIP:START -->
**2026-09-17 13:40 — IN PROGRESS — resume here**

CONTENT-LICENSES.md - the provenance record, and the measurement that makes it worth having. The owner asked for no rights problems ever and for everything verified, so the question was narrowed to the only one that changes the risk: not «is the book old» and not «is it on Shamela», but «is a modern editor's own apparatus inside the file we actually redistribute?». audit_editor_apparatus.py fetched ALL 249 catalogued books from our own bucket (Shamela untouched), read each book's own edition card for the muhaqqiq and his death year, and counted the apparatus paragraphs using the signature the azkar work had to learn - a paragraph CLOSING with (*) rather than opening with a bracketed number. Result: 71 books name no editor at all, 4 have an editor dead before 1396 AH, 126 have a modern editor whose work is NOT in the file, and 48 have a modern editor whose apparatus IS shipped. So 175 of 249 carry nothing of any modern person's, which nobody knew before, and the open list is 48 books rather than the 170 that naming an editor would have suggested. Worst is al_ijaz_fi_sharh_sunan_abi_dawud at 46.5 percent of its file being Mashhur Al Salman's, and he is alive. And shamela.ws was read directly: robots.txt 404, /page/terms 404, /page/rights 404, /page/license 404, and its own «حول المشروع» says it does not take payment from authors for publishing their books - so nothing forbids reading it and nothing grants redistribution. Four claims I had written were WRONG and were caught by checking each before committing: مصحف الشمرلي does not ship at all (six printings do, re-read from editions.json, and a memory file that said nine was corrected); editions.json carries no source or licence field for any edition; the hadith gradings have a real STATED LICENCE (MIT, huggingface meeAtif/hadith_datasets); and hadith.db has no footnote column at all, so the Risalah editor's apparatus reaches no device - the opposite of what I had written. Also recorded with a name and a deadline: madinah_gold is CC BY-NC-ND and the pipeline re-renders and crops its pages, which may be a derivative
