...
| **STATE 2026-09-22 (night, handover)** | `master` = `c3b6b51e`, pushed. **No release since v3.55.0** (tag `v3.55.0` = `bb476739`, NOT `HEAD`): everything below is unreleased and has **never been on the owner's phone** — seen on emulator-5554 only. `pubspec` still `3.55.0+57`; bump before the next release (§2.3). Verified at handover: `flutter analyze lib test` → No issues; `flutter test` → **499 pass**; hosted paths range-checked live — `hadith/hadith.zip` 200 / 22,235,941 B / `application/zip` / magic `504b`; `books/text/al_rawd_al_murbi.json` 581,137 B, `al_muwafaqat.json` 2,272,461 B, `ihkam_al_ahkam.json` 565,843 B — all 200, magic `1f8b`, **no Content-Encoding**; `mushaf/madinah_qc/001.png` 40,445 B and `604.png` 111,649 B, 200, `image/png`, magic `8950`; `quran/translations/ur.json.gz` 200, 353,189 B compressed / 1,475,080 B inflated — it **does** carry `Content-Encoding: gzip`, and that is NOT a bug here: `quran_translation_store.dart` sniffs the `1f8b` magic before inflating (trap #6 applies to books, which are clean). Measured: 7 locales × **1,742 keys**, **1** mushaf edition (`madinah_qc`, 604 pages), **229** library books, `hadith.db` 109,731,840 B / 67,153 rows / 45,219 graded, last built release APK 260,425,139 B (v3.55.0, stale), version `3.55.0+57`. |
