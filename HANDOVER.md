...
<!-- WIP:START -->
**2026-09-15 23:35 — IN PROGRESS — resume here**

makharij: the seventeen articulation points as data, graded against the book's own arithmetic. CORRECTION to the previous commit, which called هداية القاري verified: it is not. Its verification step had died on the cp1256 Unicode error, so all I actually checked was the TOC and the upload size. 203 of its 749 pages carry a title and NO text, including the whole of مخارج الحروف, صفات الحروف and التفخيم والترقيق - confirmed at the source: ajax/pageContent/22869/50 (printed p.65) returns an anchor and a copy button, nothing else. The crawl is faithful; the Shamela copy is hollow. It stays on R2 but is NOT catalogued and must not be built on. غاية المريد, which has zero empty pages, carries the same chapters and is the source used: مخارج الحروف ص126-131 read verbatim. The data is not graded against my reading of it - the book prints its own tally on p.131 («فالجوف ثلاثة أحرف، والحلق ستة، واللسان ثمانية عشر، والشفتان أربعة» = 31 over 17 makharij) and test/makharij_test.dart holds the data to those numbers. PROVEN to fail on a wrong list: dropping the س from طرف اللسان made it report «Expected 18, Actual 17, MakhrajRegion.lisan: the book says 18». analyze clean, 228 pass.
