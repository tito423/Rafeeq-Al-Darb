...
<!-- WIP:START -->
**2026-09-17 18:12 — IN PROGRESS — resume here**

checkpoint(wip): 18 of the 27 new books are in the catalogue and the library is 205 entries. book_catalog_urls_test caught a real hole the moment the entries landed: it requires every book to say SOMETHING about itself - either a written blurb under book_desc, or the generated sentence, which needs a page count to be worth printing - and my generator set neither for the books I had not written a blurb for, so their cards would have rendered a blank line where the description goes. The fix is a measured fact rather than a filler string: the highest PRINTED page number the edition reaches, read off the built pages, not meta.pageCount which counts the pages fetched and starts at Shamela's page 1 rather than the printing's. That meant reverting the 17 entries I had already written and regenerating them, which is why this commit rewrites them. The shelf test is still red at 8 of 14 books and stays red until the last six build - al_jami_li_akhlaq_al_rawi, muqaddimat_ibn_al_salah, al_risalah_lil_shafii, al_burhan, al_itqan and al_muwafaqat. Also fixed a Monitor of mine that reported the builders dead: its liveness check used pgrep, which does not exist in this Git Bash, so command-not-found made the condition true on the first pass. tasklist is the check that works here. And the six books already hosted were verified on the PUBLIC endpoint the app actually uses, not just through the S3 API: HTTP 206 on a range request, Content-Length equal to the catalogue's sizeBytes, no Content-Encoding header, first two bytes 1f 8b
