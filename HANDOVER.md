...
<!-- WIP:START -->
**2026-09-17 18:25 — IN PROGRESS — resume here**

checkpoint(wip): I HAVE TO CORRECT THIS MORNING'S HEADLINE. The rights audit reported «0 books carrying a modern editor's apparatus across all 248» and that number was wrong. Its editor detector matched eleven fixed label words; a scan of every built book's edition card found the library uses THIRTY-SIX - تعليق وتحقيق, حققه وخرج أحاديثه, دراسة وتحقيق, قدم له وحققه وعلق عليه, جمعه ورتبه ووثق نصوصه وحققه. Worse, verdict() returned NO_EDITOR_NAMED BEFORE it looked at the apparatus count, so a book whose editor the card named in an unknown form AND which carried apparatus landed in the one bucket nobody re-reads. That is the al-Adhkar shape exactly. Widened detection plus a new APPARATUS_BUT_NO_EDITOR_NAMED verdict found TWELVE such books. The worst is the one the Hajj screen reads verbatim. The only printing of «الإيضاح» Shamela has carries a SECOND living author's whole book alongside an-Nawawi's - «الإفصاح على مسائل الإيضاح» by عبد الفتاح حسين رواه المكي - and the edition card says so with «وعليه:», which nobody had read. Measured before changing anything: of the 1474 paragraphs the nineteen Hajj steps render, 280 (19.0 percent) were HIS, under a caption saying the text is an-Nawawi's. That is 1.2 broken regardless of rights. And because hajj_screen slices by paragraph INDEX inside a page, filtering renumbers everything, so remap_hajj_bounds.py re-pointed each boundary BY ITS OWN TEXT: twelve of the nineteen steps turned out to END on one of his notes and each snaps inwards by one, never outwards. No from boundary moved. My first attempt to measure the exposure said 78.6 percent and I did not report it - the stripper also edits inline markers, so surviving paragraphs fail an equality test; comparing on a marker-insensitive key gives 19.0, which is consistent with the 39 percent measured file-wide. Eleven books published filtered, tuhfat_at_talib REMOVED (45 empty pages, a 32-page run - the al_ijaz verdict). Re-audit with the widened detector: 0 and 0, for real this time. Library 204, flags 40. Only the shelf test is red, waiting on six books still building
