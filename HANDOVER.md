...
<!-- WIP:START -->
**2026-09-05 08:32 — IN PROGRESS — resume here**

P3-43 items #6 and #8 done + live-verified. #6: full-screen mushaf mode is now genuinely full-screen in both modes - new quranFullScreenProvider seam lets AppShell hide its own bottom NavigationBar while QuranScreen hides its own AppBar and bottom bar, all three chrome layers gone at once, tap-to-exit reuses the existing P3-43-#1-verified background-tap mechanism (image mode gained a narrowly-scoped onBackgroundTap solely for this, not a general tap-to-toggle it never had). Also moved the fullscreen toggle button out of the text-only toolbar block (image mode never had it) and fixed the image-mode toolbar's now-stale single-row height estimate. Found and fixed a real bug live, not by inspection: gating the AppShell nav bar on the shared flag alone made it vanish on the Home tab too, since IndexedStack keeps every tab's State (including QuranScreen's persisted fullscreen flag) alive simultaneously regardless of which tab is showing - fixed by also gating on the active tab index. #8: onboarding's real Flutter overflow (RIGHT OVERFLOWED BY 5.3 PIXELS on the bundled Hafs card's Download+Delete row) fixed at its actual shared source, MushafDownloadTile - Row-with-Spacer swapped for Wrap, same shape as P3-42's own toolbar overflow fix. flutter analyze/test clean throughout (15/15). Live-verified end to end on a clean fresh install (emulator-5554, batch26): onboarding's Hafs card renders its Download+Delete row with zero overflow, full-screen toggle hides/restores the AppBar+both bottom bars correctly in both text and image mushaf modes, and Home's own bottom nav bar is confirmed present (the regression this session found and fixed).
