...
<!-- WIP:START -->
**2026-09-05 17:31 — IN PROGRESS — resume here**

checkpoint(wip): P3-44 round-8 real-device feedback, first batch of fixes. Splash video now plays with its real soundtrack (an earlier session had muted it as a silent brand beat; owner confirmed the clip actually has sound). Home header card is now theme-aware instead of hardcoded dark in every theme, fixing it looking broken in Light theme. Location fetch now falls back through Geolocator getLastKnownPosition then this app own persisted last-good fix (7 day cache) before giving up, fixing the real indoors-at-work symptom where a transient GPS failure wiped the whole prayer-times card back to the enable-location prompt even though permission was granted and prayer times themselves already had a perfectly good cache that never got a chance to run. Mushaf fast-page-scrollbar direction now follows the app locale (RTL: page 1 physically on the right, matching a printed Arabic mushaf) instead of always plain left-to-right. Fixed a real overflow: full-screen mushaf reading had no bottom clearance reserved for the persistent page-number badge, so a page whose last line ran close to the bottom rendered underneath it - now padded. Sunnah-of-the-surahs home card decluttered per the owner own new house rule that more than 2 options on a card should collapse elsewhere - the per-surah reminder bell moved to a real Settings section. Fixed the repetitive swipe-hint text repeating on every single book and azkar page instead of showing once. System back button on any non-Home tab now returns to Home instead of exiting the app outright. Added the requested new tasbeeh istighfar preset. Added a real preview button for adhan videos, there was none before, only download and select blind. Added a new autostart and protected-apps settings card for Xiaomi, Honor, Huawei, Oppo, Vivo, and OnePlus, since standard battery-optimization exemption does not cover their own separate background-app killer - this is the concrete buildable half of the notification killed quickly report; the rest needs the owner to actually grant it and re-test on the real device before this can be marked verified. flutter analyze and test clean, 21 of 21, parity holds across 6 locales. Not yet live-verified on a real device or emulator, next step.
