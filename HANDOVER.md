...
<!-- WIP:START -->
**2026-09-05 14:52 — IN PROGRESS — resume here**

checkpoint(wip): P3-43 item #12 done and live-verified offline: root-caused the broken/disconnected Quran-mode title glyphs directly from the reference screenshot already in the repo (zoomed with PIL) rather than waiting on a new one from the owner. The isolated letterforms were a text-shaping failure, not a locale bug - real cause found by reading google_fonts 6.3.3 source: Cairo (the app entire UI font, via AppTypography) was never bundled locally, only declared through GoogleFonts.cairo, which fetches the font over the network on first use and falls back to a non-Arabic-shaping-aware system font while that fetch is pending or fails. That is also a real offline-first violation bigger than the original complaint, since it is core UI chrome text, not just downloaded content. Fixed by bundling Cairo the same way AmiriQuran already is: instanced 4 real static weights (400/500/600/700, the only ones this app actually uses) out of Cairo real upstream variable font using fonttools varLib.instancer, named to match google_fonts own asset lookup convention so existing call sites needed zero code changes, plus GoogleFonts.config.allowRuntimeFetching = false in main so a future missing weight fails loudly instead of silently repeating this bug. flutter analyze/test clean 21 of 21. Live-verified end to end on emulator-5554 with wifi and data fully disabled and airplane mode on (confirmed via adb ping failing with network unreachable): fresh install, full onboarding, and a language switch to Arabic all worked offline, then force-stopped and cold-relaunched the app to match the real reported scenario and opened the Quran tab - the title now renders as properly joined cursive Arabic script, confirmed by cropping and zooming the actual screenshot pixel for pixel. Only number 2 (real-device adhan re-verification) and number 11 (needs a real repro) remain on the P3-43 list, both blocked on the owner having the device in hand.
