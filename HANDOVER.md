...
<!-- WIP:START -->
**2026-09-17 19:43 — IN PROGRESS — resume here**

checkpoint(wip): the prayer card gets its action buttons, and I have to correct something I said an hour ago. I told him the card showing المغرب ٦:٢١ at 19:13 was a bug. IT IS NOT. It is elapsedWindow doing exactly what he asked for earlier - لما يحين وقت الصلاة يبدأ يعد عدّاد تصاعدي لحد ساعة - so for the hour after an adhan the card deliberately stays on the prayer that came in, counting UP. Salatuk beside it had already moved to العشاء, which is what made it look wrong. The rule stays; what was missing was a way past it. So: two buttons, captioned from Dart because nothing native may invent a user-visible word. فتح التطبيق always, and الصلاة القادمة only while the card is sitting on an elapsed prayer AND there is a next one - a button that would do nothing is worse than no button. It sets skip_elapsed, which post() consumes on the very next read, so the override lasts exactly one post and the rule is not changed. The other thing I got wrong and checked rather than assumed: the header read 07:58 and I could not tell what it was, so I dumped the notification instead of guessing. when=1789660260000 is Isha, chronometerCountDown=true, showChronometer=true - 19:51 minus 19:43 is 7 minutes 58 seconds. The countdown was always working; I had misread it as a clock time. What still differs from Salatuk is only WHERE it sits: Android puts a chronometer in the header timestamp slot, and Salatuk draws its own in the body in red with a minus sign, which needs a custom RemoteViews body. analyze clean. Verified on emulator-5554 at 19:43 with a real Isha at 19:51
