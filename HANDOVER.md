...
| **Released** | **v3.35.0** — the Arabic-numerals and provenance release. v3.34.0's tag and release are deleted, so the repo holds one release as the owner asked |
| **App version** | `pubspec.yaml` `3.35.0+36`; `AboutScreen.appVersion` `3.35.0` — a test checks the two against each other |
| **On the owner's Honor** | still the signed **3.25.0** build. **Nothing from v3.26.0 to v3.35.0 has run on his phone.** Everything below was seen on `emulator-5554` |
| **Signing** | `sign_release.py` printed **`OK: rotated`**, and the update path was PROVEN rather than asserted: the previous signed release (v3.34.0) was installed, then v3.35.0 installed over it with `adb install -r` → **Success**. No uninstall, so a device holding hundreds of MB of downloaded content keeps it |
| **Library** | **213 books** in 9 shelves — tazkiyah 107, hadith 29, adab 22, **talibIlm 14**, tarikh 12, fiqh 10, aqidah 8, seerah 8, tafsir 3. **43** carry `editorNotesRemoved` and say so in the reader |
| **Locales** | 7 × **1,523** keys each, identical key sets (the parity test enforces it) |
| **Verified 2026-09-18** | `flutter analyze lib test` → **No issues found** · `flutter test` → **358 passed** · release APK **207.4 MB**, `versionName=3.35.0 versionCode=36` read back from `dumpsys package` on the device · **13/13** hotlinked card backgrounds answer a range request (2 were 404 this morning) · hadith pack **22,235,941 bytes** measured with `head_object` · six ruqyah objects on the bucket, every byte size matching the catalogue |
| **Arabic numerals** | this release's main body of work. **~70 sites** across the app printed Latin digits into Arabic sentences. Fixed at the seam — `trn()` / `pluralN()` / `uiLanguageCode` in `core/utils/digits.dart`, written from the one place that already tracks locale (`RafeeqApp.build`) — not screen by screen, because seven screens were patched by hand first and the eighth kept producing more |
| **Bidi** | the «a / b» pair reversed in **16** places, including the mushaf page badge (the original report) and the tasbeeh counter. Both had been "fixed" once already by splitting into three `Text`s in a `Row` — which trades a bidi reorder for a `Row` reorder and renders identically wrong. `ratio()` is the fix; `test/ratio_call_sites_test.dart` bans every shape of it, including the separator-as-its-own-widget form that shipped past a guard written to catch it |
| **Provenance** | CONTENT-LICENSES corrected: the ruqyah paragraph said «the five recordings … from the Internet Archive» and there are **six**, the sixth supplied by the owner and not from the Archive. Unsplash added to the Sources screen — it supplies the Azkar and New Muslim card photographs and had never been credited on a screen that opens «كل محتوى في التطبيق مأخوذ من المصادر التالية» |
| **Emulator** | `hw.ramSize` raised **2048 → 4096** (config backed up beside it). At 2 GB the app ANR'd for 13.9 s on launch — 109% kernel time, 4,197 major faults, kswapd at 34%, 46 MB swapped. That was the host, not the app, and it made the screen sweep impossible until it was fixed |
