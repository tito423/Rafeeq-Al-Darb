...
<!-- WIP:START -->
**2026-09-15 21:58 — IN PROGRESS — resume here**

verify on the owner's Honor: Hajj counters, ayah-recitation screen, tajweed listen. Hajj tawaf 1/7 -> marker back at the Black Stone with one full ring drawn -> 7/7 -> completed; sa'i marker alternates Safa->Marwa correctly and finishes; jamarat 7+7+7 with each pillar turning green and no pebble left on it; the Umrah tab switches art and step list. Ayah screen: play glyph now visible (gold disc, dark glyph), empty rings have a visible track, and a finished surah shows a check rather than a stranded pause. Tajweed listen works from a COLD start without opening downloads first - twice, logged resolving everyayah Minshawy_Mujawwad and dumpsys audio state:started; my first two attempts failed because the tap missed, not the code, and I said so rather than filing a bug. Left one debugPrint on the all-sources-failed path: every caller swallows the failure, so a dead listen button was indistinguishable from a missed tap, which is exactly what misled me. code_layout_test then failed because that comment pushed ayah_audio_service.dart past its 810-line ceiling - trimmed to fit rather than raising the ceiling, as the test's own message says. analyze clean, 221 pass
