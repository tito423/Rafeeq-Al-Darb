...
<!-- WIP:START -->
**2026-09-17 14:19 — IN PROGRESS — resume here**

the 48 books that were shipping a modern editor's own work: 47 filtered, 1 removed, and the result seen on a device. strip_editor_apparatus.py takes the apparatus out and then MEASURES whether a book is left - and the measurement that decides it is WHERE the empty pages fall, which the first version of the report missed: a blank run at the FRONT means the editor's introduction is gone and the author's book now starts where he starts, which is exactly right, while a run in the MIDDLE means the book has been gutted. juz_bay_ummahat_al_awlad loses 24 consecutive pages and keeps 82.7 percent of its text, and those two numbers only make sense together once you know the 24 are his front matter. On that split: 34 books keep 95 percent or more with no interior gap, 11 keep 70-95 with small ones, and 3 needed reading. Two of the three were fine - masalah_fil_kanais loses five scattered footnote-continuation pages, and umdat_al_ahkam loses the editor's title page and his ijaza appendix while all 428 pages of the hadith book survive. The third was not: al_ijaz_fi_sharh_sunan_abi_dawud keeps 47.7 percent, empties 83 pages INSIDE the book with a run of 20, its printed page 51 is a row of dots because that leaf is all footnote, and the pages that survive still say «النسخة التي اعتمدناها في التحقيق» - it is Abu Ubayda Mashhur Al Salman's reconstruction, he is alive, and a book that has to lose half of itself is not that book. Removed from the catalogue and the bucket, the way the v3.29.0 purge sent the other 23. The other 47 were uploaded over the same keys, gzip with no Content-Encoding as policy says, and sizeBytes and pages were rewritten in book_catalog.dart from the bucket's own READBACK rather than from what we meant to send. All 47 originals are backed up outside the repo first. Then two things fell out that are worth more than the change itself: re-running the azkar extractor against the newly filtered al-Adhkar gives byte-identical output - 338 sections, 1,392 paragraphs, all 22 curated shas still matching, and «0 footnotes dropped» because they are already gone, which proves the two filters agree exactly; and on the emulator al-Adhkar opens on page 3 with an-Nawawi's own muqaddima and WITHOUT al-Arna'ut's footnote that used to sit on that page, with the corrected source line reading «تحقيق عبد القادر» - the one claim I had said was unverified on a screen. 248 books, analyze clean, 287 tests pass
