...
<!-- WIP:START -->
**2026-09-09 15:15 — IN PROGRESS — resume here**

checkpoint(wip): The gilded mushaf highlights ayahs - FIVE of nine printings now do. The claim that kept it out for three sessions was a measurement of the wrong thing: «madinah_gold sets 6 lines on its page 2 where the Madinah mushaf sets 15». Page 2 of ANY Madinah printing is al-Baqarah illuminated opening and sets SIX lines - the Hafs polygon layer itself says so, and test/mushaf_polygon_fit_test.dart now counts them the same way the fitter does and pins 7 lines on page 1, 6 on page 2, 15 on body page 3 and 13 on page 50 (fifteen slots less the surah header and the basmalah). Fifteen is what a BODY page sets, and madinah_gold sets exactly fifteen ink runs on every body page sampled, stable across nine threshold combinations. All 604 pages fitted DIRECTLY, none filled from the frame, median residual 0.00294 of page height (about 6px on a 2034px page), 95th 0.00933, max 0.01552. Three things the fitter needed, each opt-in per edition so the three printings already shipped re-fit BYTE-IDENTICAL (proven by re-running qatar, kuwait and madinah_night and diffing): (1) a saturation mask on the panel path, because the leaf is cream at saturation ~72 and darkness alone cannot separate text from gold illumination; (2) merge_close, because a single Arabic line breaks into two row-runs where only the diacritics reach the upper rows - merged, both openings read 6 lines at every min_run from 0.008 to 0.014 while the raw count wandered between 6, 7 and 8; (3) drop_target, for a line the page really sets but no ink threshold can see - al-Fatiha basmalah is drawn in GOLD, yet it is ayah 1:1 and carries a polygon, so the hafs slot is dropped and the affine fitted on the six visible lines places it. Also a one-point robust refit: page 50 first slot is a header cartouche whose ink centre sits 0.021 high, which dragged the page affine, failed the outlier cut and left it floating a quarter of a line off. Proof overlays rendered and LOOKED AT for pages 1, 2, 50, 300 and 604 - the extrapolated basmalah lands on the basmalah, page 604 skips three surah headers and three basmalahs correctly. check_mushaf_pages.py over all 604: no defects, the only two flagged are the illuminated openings themselves. analyze clean, 41/41
