...
<!-- WIP:START -->
**2026-09-13 01:08 — IN PROGRESS — resume here**

the tajweed course is in More, and its lessons were WRONG until they were read against the real book. VERIFIED on emulator-5554: the card opens, the book downloads itself once, all 23 lessons list with a live progress bar, lesson 6 الإقلاب opens exactly on its own heading, استمع plays the ayah (audio dumpsys: AudioTrack state:started, USAGE_MEDIA 44100) and the button turns into إيقاف which really stops it, and إتمام الدرس moved the header to أنجزت 1 من 23 with the gold tick on that lesson. WHAT WAS WRONG: the first cut addressed each lesson by PRINTED PAGE, and this book does not start its topics at the top of a page - الإقلاب begins seven paragraphs down page 9 under the tail of إدغام بلا غنة, المد الجائز المنفصل three paragraphs down page 20 under more examples of المتصل, and التفخيم and الترقيق were simply swapped (page 27 is الترقيق, page 28 is التفخيم). Every one of the 23 spans is now an explicit fromPage:fromPara - toPage:toPara range read off the real text paragraph by paragraph, and the first and last line of each was printed and checked. test/tajweed_course_test.dart guards the invariants and was PROVEN to fail on the old mistake - reintroducing fromPara 0 on المنفصل made it report «المد الجائز المنفصل starts inside المد الفرعي: الواجب المتصل». Progress is stored by lesson TITLE (tajweed.done_v2), not by index, so the next boundary fix cannot slide a reader's ticks onto other lessons. Not a word of the science is written by me: the text is «تيسير أحكام التجويد - المستوى الأول» for د. يحيى الغوثاني downloaded from R2, the source line names it on screen, and the 12 audible examples were each matched against quran_local.db before being written down. Also in this commit: the on-this-day datasets finished building (ar 200KB, en 207KB) but are NOT yet wired into pubspec assets, and the الألباني crawl finished - 10 books built and uploaded, still uncatalogued and with ugly transliterated slugs.
