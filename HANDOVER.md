...
<!-- WIP:START -->
**2026-09-12 12:36 — IN PROGRESS — resume here**

batch 11 stage 10: the ayah card's reciter chip showed a microphone and a chevron and NO NAME, which is the one thing the owner asked it for. Seen on emulator-5554: the header Row already carried the ayah badge, the surah name, «تلاوة الآية», the expand button and the overflow menu, so the chip's Flexible name collapsed to zero width - an icon that does not say who. Moved under the surah title where it has the room; the card now reads «محمد صديق ال…» and a tap opens the same picker the page's recitation bar opens, verified on the device. Also verified in this pass: NO Latin transliteration under the ayah on an Arabic UI, and the Sources screen's new rows render with the licence each item actually states (مصحف دولة الكويت - وزارة الأوقاف، لا يحمل المجلد أي إشعار حقوق وقد فحصت مقدمته وخاتمته; مصحف المدينة الليلي - مجمع الملك فهد) plus the YouTube avatar-mirroring credit. That is the third defect this batch that passed analyze and the full suite and was only visible by opening the screen - after the tutorial's dead skip button and the unreachable channel list. analyze clean, 197 tests pass.
