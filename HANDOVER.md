...
<!-- WIP:START -->
**2026-09-05 21:49 — IN PROGRESS — resume here**

P3-45: v2.1.5 real-device round. Fixed the Home prayer card generic-error crash by root-causing a flutter_local_notifications persistence bug and guarding every plugin call site (adhan scheduler, prayer status notification, adhan alarm service including the full-screen alert's own Stop button) so notification failures never take down real prayer times or leave Stop unresponsive. Fixed the header text wrapping mid-word in fr/pt by dropping the Arabic-only font for other locales plus a hard maxLines/ellipsis safety net. Root-caused and fixed a systemic locale-staleness bug: const widget instances skip Flutter's own build() call on locale change since .tr() has no BuildContext dependency - fixed in Settings (Permissions, Sunnah reminders) and, more importantly, in AppShell's IndexedStack where 5 of 6 bottom-nav tabs had the identical bug. Fixed Downloads screen navigation (Hadith/Books rows only closed Downloads instead of reaching AppShell; Adhan row was a dead end) and removed the hard portrait-only orientation lock entirely, verified working in landscape. Removed the splash screen feature outright per direct request: deleted the whole splash feature folder, its Settings toggle, and ~5MB of now-unused branding assets. Investigated but did not band-aid a real download-hang-when-backgrounded report - traced to the app having no genuine Android foreground service for downloads, flagged as a real follow-up feature rather than guessed at. flutter analyze/test clean throughout (21/21), live-verified end to end on a full release rebuild+reinstall cycle.
