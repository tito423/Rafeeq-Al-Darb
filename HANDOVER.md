...
<!-- WIP:START -->
**2026-09-09 15:19 — IN PROGRESS — resume here**

checkpoint(wip): Kuwait two illuminated openings are fitted - the gap NEXT_SESSION_PROMPT section 2 left open is closed, and the reason it stayed open for two sessions was that darkness was the wrong test. The Kuwait illumination is COLOURED (red, teal, gold, violet) and the Quranic ink is neutral brown on a near-white panel, so no luminance cut can separate them while also separating the seven lines - a saturation mask can, and does. At ink_max 130 with sat_max 40, page 1 reads 7 lines and page 2 reads 6 across every min_run from 0.012 to 0.02; over a 60-combination sweep page 1 answers 7 in 36 of them and page 2 answers 6 in 25, and the chosen point sits in the middle of both plateaus rather than on an edge. Merging split runs is the other half. Proof overlays rendered and LOOKED AT: on page 1 ayah 1 sits on the basmalah and ayah 7 spans the two-line wrap of صراط الذين to ولا الضالين; on page 2 the five ayahs each bracket their own lines. The 602 pages Kuwait already shipped are byte-identical - checked entry by entry before editions.json was rewritten, and the script changes that made this possible (sat_max, merge, min_run, drop_target, robust) are all opt-in per edition, proven by re-running qatar, kuwait and madinah_night and diffing the output. So all four per-page printings now carry an affine for every one of their 604 pages, and the test asserts each opening uses its OWN affine rather than the body one - an entry equal to page 3 would mean the opening was never measured. analyze clean, 41/41
