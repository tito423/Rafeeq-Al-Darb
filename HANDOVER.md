...
<!-- WIP:START -->
**2026-09-03 14:20 — IN PROGRESS — resume here**

P2-12 DONE (Sunan as-Suwar card) + emulator-verified: sunan_suwar_catalog.dart (4 surahs 2/18/67/32, sourced virtue notes - never invented, page ranges resolved dynamically from mushafDataProvider.surahStartPages, never hardcoded), single_surah_screen.dart (locked reader: reuses MushafTextPage/MushafPageView, PageView bounded to the surah's own pages, restricted toolbar - no surah/juz/goto/search/edition), sunan_suwar_reminder_service.dart (weekly via DateTimeComponents.dayOfWeekAndTime) + sunan_suwar_store.dart (per-surah persisted {weekday,time}) + sunan_suwar_navigation.dart (live-tap open, mirrors adhan_navigation.dart), sunan_suwar_card.dart (Home middle, under KhatmaCard). +19 keys x5. Verified live: all 4 real surah names+notes render (AmiriQuran, ﷺ glyph correct), tapped al-Baqarah -> locked reader opened page 1/48 with restricted toolbar, paged to exact last page 48/48 showing Baqarah's real closing ayahs - confirmed the boundary is real (DB-derived) and doesn't leak into Aal-Imran. Real ADB-testing lesson (not a code bug): the disabled boundary button looked unresponsive across 50 precise taps - turned out Row's children reverse under RTL Directionality, so I was tapping the correctly-disabled prev button the whole time, not a broken next button. Owner's order: P2-11(done)->P2-12(done)->P2-9(done)->next P2-10->P2-13->P2-7(last). analyze clean, test 13/13.
