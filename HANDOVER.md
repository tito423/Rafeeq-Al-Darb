...
<!-- WIP:START -->
**2026-09-17 19:17 — IN PROGRESS — resume here**

checkpoint(wip): the Quran page is full screen always, and the controls float over it. Tap shows and hides MushafChrome - a blurred glass panel carrying surah, juz, page and the four actions - and it takes NO layout space, so the mushaf keeps 100 percent of the screen either way. No new gesture was invented, deliberately, because he warned about collisions: the page already owns a tap, a long-press to select an ayah, a horizontal swipe and a vertical scroll, and the tap's old meaning (enter/leave full screen) was freed by full screen becoming permanent. Colours come from the mushaf theme, not the app - paper tinted for the glass, ink for icons and text, gold for the active state - and the ToolbarActions are wrapped in a Theme that hands them the page's ink, because otherwise they colour themselves from the app scheme and would be black icons on a charcoal mushaf. I ALSO CAUGHT MYSELF RUNNING dart format ON quran_screen.dart: it reflowed 8 hunks of code nobody had touched and turned a 5-hunk change into a 213-line diff, so the file was reverted and the edits re-applied by hand. The file is grandfathered at 1047 lines and may not grow; it is 1045 now, paid for by deleting the old black-and-white exit circle and by tightening my own comments rather than raising the ratchet. AND A NEW BIDI TRAP, found by zooming into the screenshot instead of squinting at it: the page pill read 604 / 1. Written as one string, ١ / ٦٠٤ flips EVEN IN AN LTR PARAGRAPH - neither the ltr() isolate helper nor textDirection ltr fixed it, both were tried on the device first. Arabic-Indic digits are bidi class AN, and rule N1 treats AN as R when resolving the neutrals BETWEEN two numbers, so the separator goes right-to-left and the numbers swap around it. This is not trap 16; that was a weak numeral taking direction from a strong neighbour. Fixed structurally with three Text widgets in a Row - no shared paragraph, no neutral to resolve. Verified on device: it now reads ١ / ٦٠٤
