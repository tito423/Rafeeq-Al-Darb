...
<!-- WIP:START -->
**2026-09-09 15:43 — IN PROGRESS — resume here**

VERIFIED ON emulator-5554, not in my head - and the run found one more real bug, which is why it was worth doing. My splash fix was WRONG about the cause: a fresh install still put the POST_NOTIFICATIONS dialog over the playing splash video. logcat named the culprit - START REQUEST_PERMISSIONS from uid com.tito.rafeeq_aldarb, 4.8s after the activity started, squarely inside the ~8s video. It was DownloadEngine.ensureInitialized, whose own comment claimed that asking there meant the first download prompts rather than the app demanding it at launch - untrue, because main calls resumeFromBackground which calls it. The ask is now its own idempotent method, resumeFromBackground passes askForNotifications false, and the silent per-ayah cache does too. Rebuilt, re-cleared data, relaunched: no REQUEST_PERMISSIONS in the log during the splash at all, and location then notifications both come up AFTER it, over onboarding, in that order. Also seen and photographed: a fresh install opens on the DAY theme - المظهر shows نهاري selected and theme_variant_v2 is absent from prefs, so that is the default and not a stored value; the splash sound switch sits under the video switch, mutes with a struck-through speaker icon, and persists as splash_video_sound_v1=false. Recitation: started on al-Fatiha at 1:1, tapped ayah 5 and got 1:5 with the highlight on إياك نعبد, tapped ayah 2 and got 1:2 with the highlight spanning its two-line wrap. Both of those used to stop the recitation dead. Long-pressed a verse mid-recitation and the sciences sheet showed a PLAY button where it used to show STOP; pressing it closed the sheet and moved the recitation to 1:6. The gilded mushaf highlights on the device: page 77 of an-Nisa with the recitation on 4:2, the highlight ending exactly at the ayah marker across a two-line wrap, on a page I never inspected while fitting; and tapping the GOLD basmalah on its page 1 - the line no ink threshold can see, placed by extrapolation - opens the sciences sheet on 1:1. HONEST GAP: I could not reproduce the owner background hang on this emulator. am stopservice on the AudioService did not release the player - audio_service restarted itself and the recitation kept advancing 2:2 to 2:3 - and cutting the network did not stall it either, because played ayahs are cached to disk. What I DID verify is the recovery half: with the network off and a surah with nothing cached, the reader now says «تعذّر تشغيل التلاوة. تأكد من الاتصال وحاول مرة أخرى» instead of ending in silence, which proves both attempts ran and the second was on a freshly created player; and with the network back, recitation started again on that recreated player at 4:1, which proves just_audio_background accepts a new player after a disposed one - the single riskiest assumption in the whole fix
