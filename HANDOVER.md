...
<!-- WIP:START -->
**2026-09-08 15:49 — IN PROGRESS — resume here**

Add Urdu as a full app language and turn the two long Adhan sections into their own screens. Urdu: a complete ur.json with all 562 keys translated, verified key-for-key against en.json with no missing, extra or empty values. Religious content that already stays Arabic in every other locale (the seven tasbeeh phrases) stays Arabic here too, by the same convention. Registered in main.dart's supportedLocales, in both the settings and onboarding language pickers, and added to translation_parity_test.dart so the suite now enforces seven locales instead of six. Urdu is right-to-left like Arabic, so the shell mirrors through the existing machinery with nothing special-cased. Adhan settings: the default-adhan list and the per-prayer settings were behind ExpansionTiles, but expanding a ten-item list inside an already-scrolling settings page just relocated the wall of content it was meant to fix - the list opened squeezed between other cards with its own scroll fighting the page's. Each now opens as its own full-screen page via a new _openFullScreen helper, so the list gets the whole viewport, while the collapsed row still shows the current pick so the common case needs no navigation at all. flutter analyze clean, flutter test 25/25.
