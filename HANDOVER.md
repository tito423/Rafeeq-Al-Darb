...
<!-- WIP:START -->
**2026-09-05 12:48 — IN PROGRESS — resume here**

P3-43 item #2 root-caused live on the owner's real device (Honor X9c, Magic OS, Android 16/API 36) via adb, and a real fix built. Part 1 confirmed via 'adb shell appops get': USE_FULL_SCREEN_INTENT was never granted on this phone (the Android 14+ per-app toggle beyond the manifest permission) - the in-app detection/prompt for this already exists correctly from P3-19, owner just hadn't granted it; granting it directly via adb confirmed the full-screen launch itself then fires. Part 2: even with permission granted, the full-screen alert opened showing whatever screen the app was already on instead of the adhan alert screen - read flutter_local_notifications 16.3.3's actual Android source to confirm the plugin's own Intent-based navigation path (getNotificationAppLaunchDetails/onNewIntent) is structurally correct, so this points at the real Intent not surviving this OEM's locked-screen full-screen-intent launch faithfully - a known class of skin behavior no app code can force correctly. Fixed with a device/OEM-agnostic safety net instead of a device-specific patch: new AdhanAlarmService.findActiveAdhanPayload() asks Android's own getActiveNotifications() directly on every app resume, and navigates to the real alert screen if an adhan notification is genuinely posted, regardless of how the app came to foreground; openAdhanFromPayload gained duplicate-push protection so this coexists safely with the existing Intent-based path. flutter analyze/test clean (15/15). Not yet re-verified live - the device's USB/adb connection dropped while locked (a separately-observed real quirk of this phone) right as the fix was being built; needs a retest on reconnect. PHASE3.md also records the owner's broader ask (all Android 5+, all OEM skins, tablets, TV) honestly: current minSdk already covers Android 5.0, this fix itself is a generic reliability improvement not Honor-specific, and the concrete next layer (per-vendor autostart/protected-apps settings deep links for Xiaomi/Huawei/Honor/etc, separate from the already-existing battery-optimization exemption) is flagged as a real, scoped, not-yet-attempted follow-up rather than falsely claimed as done.
