...
<!-- WIP:START -->
**2026-09-09 07:36 — IN PROGRESS — resume here**

Bug found by running the release build on the device, not by reading it: sahih_as_seerah_albani shipped with a BLANK author name - an empty row in the 'authors' list and a card reading '. died 1420 AH' with a leading separator. Cause: Shamela's book card for id 592 carries no 'al-muallif:' line at all. Al-Albani abridged that book rather than wrote it, so the card names him on a 'lakhkhasahu ... wa allaqa alayhi:' line, the crawler's author parser found nothing, and add_seerah_catalog_entries.py wrote the empty string straight into the catalogue. Fixed from the source's own card, not invented. The generator now refuses an empty author and prints the edition card so the next person can read the real name off it - an empty author is a sourcing question for a person, not something to paper over. Also verified on the device: the six-edition sheet with Mushaf Qatar and its real printed cover, its page 50 (Al Imran, juz 3) and page 604 (al-Ikhlas/al-Falaq/an-Nas, juz 30) both streaming from R2 with the correct running header at both ends of the range, so the page offset is right. analyze clean, 30/30
