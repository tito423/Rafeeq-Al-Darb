...
<!-- WIP:START -->
**2026-09-13 02:18 — IN PROGRESS — resume here**

AGENT_TASK_PROMPT.md - the guarded brief the owner pastes to another agent. He is handing the remaining work to a second agent and has been burned once already: he gave Gemini Pro 3.1 a prompt from here and it changed the design anyway. He also told me he does not know what a branch or a commit is and does not want to learn right now, so every safety net in this file has to work WITHOUT him running or reading anything - which is why the final section makes the AGENT print the three check commands verbatim in its own last reply, including the one whose output must be empty (a diff of lib/core/theme and pubspec.yaml against master). The frozen list is explicit about the theme, existing screens, existing translation keys, dependencies, formatting, secrets and built assets, and section 5 forbids the obvious escape hatch: deleting, skipping or editing a test to make it pass, with the current count 212 written down so a drop is visible. It also carries the two rules that actually break this project - nothing fake, and nothing is done until it has been seen on a device - each with a real example of what it cost. Committed to the repo rather than left in chat so it survives the session.
