...
<!-- WIP:START -->
**2026-09-12 04:28 — IN PROGRESS — resume here**

batch 11 stage 4 (code only, not yet on a device): the guided tour. TutorialScreen is a twelve-chapter PageView in the bottom bar's own order (welcome, home, quran, recite, prayer, azkar, tasbeeh, library, more, themes, offline, sources) with a painted turning rub-el-hizb medallion per chapter taking that chapter's accent, a parallax between medallion and text, a segment rail, and a skip control on every page including the last. Drawn rather than screenshotted on purpose: four themes, seven languages and two mushaf modes mean a screenshot is wrong for most readers the day it is taken. One exit path (_finish) so the back gesture records seen exactly like the skip button does - PopScope canPop:false. State is two separate facts: seen-once (so an existing install discovers the tour) and every-launch (off by default, the option the owner asked for). AppShell opens it on its first frame when due, before the 900ms permission prompt so the order matches a first run. TutorialEntryCard sits first in المزيد carrying both play-now and the every-launch switch. 33 tutorial keys written in all seven languages, each body describing what is actually in the build. analyze clean, 192 tests pass.
