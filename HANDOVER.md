...
<!-- WIP:START -->
**2026-09-17 18:51 — IN PROGRESS — resume here**

checkpoint(wip): full audit, first pass - and it found a real security issue. SYSTEM_ALERT_WINDOW, draw over other apps, the most abusable permission Android has, was DECLARED on an app distributed as a sideloaded APK and nothing in the repo used it. Its comment called it a fallback path for the Adhan on OEM skins that throttle full-screen intents; there was no fallback - no canDrawOverlays, no TYPE_APPLICATION_OVERLAY, no ACTION_MANAGE_OVERLAY_PERMISSION anywhere. It was declared for code nobody wrote. Removed, 20 permissions down to 19, with a note in its place saying do not put it back without writing the code first. What actually crosses the lock screen is untouched: a fullScreenIntent notification launching AdhanActivity, which sets FLAG_SHOW_WHEN_LOCKED and FLAG_TURN_SCREEN_ON and FLAG_DISMISS_KEYGUARD on its OWN window, gated by three other permissions that all remain. MY FIRST AUDIT SCRIPT PASSED IT, and that is worth recording: its evidence needle for that permission included the bare word overlay, which matches page_overlay.dart and Flutter's own Overlay class and a dozen colour names. A check that can pass on an unrelated word is not a check - same family as trap 47. android_permissions_test.dart uses the actual API symbols instead, strips XML comments first so the removal note cannot satisfy its own search, and I proved it by re-adding the permission and watching it fail before removing it again. It also pins that the adhan still has the three permissions and three window flags it needs. Everything else in this pass was clean: .env untracked, no committed secret, no verify=False outside comments forbidding it, no plain http in app code, the only exported components are MainActivity and just_audio_background's two, all 209 catalogue sizes match the bucket exactly, all six mushaf editions answer 206 with the right content type on first, middle and last page, both recitation CDNs answer, and hadith.db has ZERO hadiths with a grade but no grader across 67153 rows
