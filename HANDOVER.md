...
<!-- WIP:START -->
**2026-09-17 17:06 — IN PROGRESS — resume here**

checkpoint(wip): the three adhans he supplied are unnamed now, azan13 is back, and the dawah lists are cut to what he named. azan11/12 are the owner's OWN voice and azan13 is a recording he says he has permission for, so all three read أذان ١/٢/٣ - a display choice, not a provenance one: each entry's source line still records where the file came from, including that azan13 is used by permission. azan13 came out of removedAdhanIds the same day it went in, which is the round trip that set exists for - the mp3, the loudness measurement and the phrase timings were deliberately left in place, so restoring it cost one JSON entry and one id. removed_adhans_test.dart now pins it from the other side (in the catalogue AND not declared removed - being in both is the state that crashes the sheet) and adds a check that adhans.json contains neither of the two personal names, because a rename that leaves the old name in a neighbouring field is the Крепости мусульманина defect again. Channels 22 -> 9, sites 7 -> 3, and the 17 orphaned locale keys went with them in all seven files. The first pruning script SPLIT the Dart list on a lookahead whose comment-block part could match zero lines, so every line inside a comment was a split point and the first kept entry lost its own constructor line; flutter analyze caught it, the file was restored from git and the second script walks the list instead. channels_reachable_test.dart asserted a floor of 20 channels and named two letter-tile channels that are gone - both updated with the reason. And the Sources screen was STALE: it still credited التحقيق والإيضاح لابن باز although the Hajj guide moved onto النووي's الإيضاح when the rights question was settled, so the app named a book it reads no word from. Corrected to shamela.ws/book/96232, which answered 200. analyze clean, 319 tests
