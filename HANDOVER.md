...
<!-- WIP:START -->
**2026-09-08 13:47 — IN PROGRESS — resume here**

Live-verify the three new mushaf printings and the 47-language translation picker on the emulator, and fix a stale download-size label. Emulator verification on a release build: the mushaf picker now lists all seven editions with correct covers, medallions and per-printing page counts (Shamarly 521 on a black-and-gold board, Madinah illuminated 604 on antique gold, Indo-Pak colour-coded 564 on teal); selecting Shamarly renders its real page 1 streamed from R2 with the blue and orange ornamental header intact. Translation tab verified end to end: the dropdown lists the six bundled languages first with no size, then every downloadable language by its own native name with its gzipped size; picking Deutsch downloaded it and immediately rendered Bubenheim and Elyas, proving the whole path (R2 fetch, gzip sniff, SQLite insert, per-ayah read). Fix found by that same verification: after a language finished downloading, its size label stayed on the closed dropdown, still implying it needed fetching, because the dropdown was built outside the installed-languages listener. The tab body was extracted to _tab and the whole thing now rebuilds on QuranTranslationStore.installed, so the label clears the moment the download lands; the now-redundant inner listener was removed. flutter analyze clean, flutter test 25/25.
