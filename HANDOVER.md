...
<!-- WIP:START -->
**2026-09-03 12:37 — IN PROGRESS — resume here**

P2-8 #9 DONE + emulator-verified: added es/ru/pt Quran translations (Cortes/Kuliev/El-Hayek from alquran.cloud, same trusted source as en/fr/ur) so Rafiq's own es/ru/pt UI locales finally get a translation in their own language. scripts/ingest_translations.py extended (EDITIONS list +3) + a real bug fixed in it (an unconditional cleanup-step query on the now-already-dropped ayah_sciences table crashed and rolled back a whole run's real inserts on any second run - moved the commit earlier so that class of bug can't recur). SciencesRepository.supportedTranslationLangs +3, translation copy-stamp bumped sciences-v2->v3 so existing installs actually pick up the new rows, _labels map in the ayah sheet's translation tab +3. Verified live: dropdown lists all 6 languages, Russian selected showed real Cyrillic text with correct translator attribution. quran_sciences.db 23.2MB->26.1MB. This closes out P2-8 entirely (all 6 owner-picked items done); the 3 research-only items (#8 tajweed/#12 group-khatma/#13 radio) stay research-only per owner's own scope for this round. Also fixed a real bug found live: 'معاني الكلمات' word-meanings tab paired with i'rab tokens for readability, researched+rejected a copyrighted Arabic-gloss source per owner's explicit no. analyze clean, test 13/13.
