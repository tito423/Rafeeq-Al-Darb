...
<!-- WIP:START -->
**2026-09-05 14:22 — IN PROGRESS — resume here**

checkpoint(wip): P3-43 item #16 done and live-verified: 3 more real Shamela titles added to the library, one per already-established author, growing the same safe-default set rather than guessing at the owner open-ended download-them-all scope. qasr_al_amal (Ibn Abi al-Dunya), al_hasanah_wa_al_sayyiah (Ibn Taymiyyah), adab_al_nafs (al-Hakim al-Tirmidhi) - each real Shamela id verified directly against its own landing page before being added to build_book_text.py BOOKS dict (now 11 entries). All 3 built clean (printMatches/printReliable both True, zero empty pages, real toc/sample text), added as text-only LibraryBook catalog entries under the tazkiyah category, uploaded to R2 via a new dated script r2_upload_books_p3_43.py, verified with head_object size match and a live curl check against the real public URL returning HTTP 200 with matching Content-Length. flutter analyze/test clean 21 of 21. Live-verified on emulator-5554: all 3 appear correctly in All books and grouped correctly under Categories; downloaded qasr_al_amal, retried past one transient emulator DNS hiccup, opened it in the text reader and confirmed real fahras, printed page number, full hadith text with tashkeel, source label, and page slider all render correctly end to end. Only number 2 real device adhan re-verification and number 12 owner screenshot needed remain on the P3-43 list, both blocked on the owner rather than more work this session.
