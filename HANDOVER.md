...
<!-- WIP:START -->
**2026-09-08 05:27 — IN PROGRESS — resume here**

Flatten Warsh/Qalun mushaf scans onto white, fixing a black-on-black page, and live-verify the whole change set on the emulator. Live verification caught that the Warsh pages, though loading correctly from R2, were very nearly invisible: the upstream Quran-for-Android scans are transparent glyph masks (background RGBA 0,0,0,0 with black letterforms) because that app tints them per theme, and dropped onto this app's dark navy reader they rendered black on black. Fixed in the data rather than the renderer - r2_upload_mushaf_editions.py now flattens every page onto white before upload, which is also exactly what the existing Tajweed scans already are (opaque white RGB), so all three raster editions now behave identically and the reader needs no tint branch. Flattening is done in palette mode so it is free: page 90 went 309108 to 309668 bytes. Chose this over a srcIn theme tint in the reader specifically because a tint would have flattened away the red rubrication that both the Warsh and Qalun printings use. Also parallelized the uploader (16 workers, per-thread boto3 clients since boto3 clients are not documented as thread-safe) after the sequential version measured about 7 pages a minute, i.e. roughly 3 hours for both editions, and added a --force flag to re-upload pages already present when the transform itself changes. Re-uploaded all 1208 pages flattened, 0 failures, and verified pages 1/90/302/500/604 of both editions now come back opaque white through the public endpoint. Emulator verification on a release build with cleared app data: the picker shows exactly the four shipped editions with correct leather covers and medallions, Warsh and Qalun both display the numbering-divergence notice, selecting either renders real crisp black-on-white pages with the red rubrication intact and hides the text/image toggle, tapping a prayer chip on Home opens Adhan settings, and the new auto-location card toggles on to reveal all six intervals (30 minutes, 1, 2, 5, 12, 24 hours) with the selection persisting across leaving and re-entering the screen. flutter analyze clean, flutter test 25/25.
