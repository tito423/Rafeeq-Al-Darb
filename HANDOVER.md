...
<!-- WIP:START -->
**2026-09-03 14:40 — COMPLETE**

P2-10 DONE except owner-blocker: ARCHITECTURE.md written (layers, Riverpod shapes, data sources, IndexedStack rationale, notification-service split, copy-this-pattern table); real size measured (41.8/43.2/44.8 MB, +6MB vs Phase-1, documented not hidden); security re-swept clean; found+deleted genuine dead code (download_tile.dart, never imported); found+fixed a real usability gap - all 8 AsyncValue error branches were dead-ends with no retry, added core/widgets/error_retry.dart (Retry button -> ref.invalidate) and wired every one. analyze clean, test 13/13. Remaining: owner must supply real release keystore (unchanged blocker). Next: P2-13 (random-hadith card, needs graded hadith.db rebuild first per owner's Option A decision), then P2-7 last (video Adhan real-device verify).
