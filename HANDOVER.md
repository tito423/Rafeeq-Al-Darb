...
<!-- WIP:START -->
**2026-09-03 20:53 — IN PROGRESS — resume here**

P3-4 Home header card DONE + LIVE-VERIFIED on emulator-5554: replaced the old AppBar title ('رفيق الدرب') + time-of-day greeting text entirely with a new fixed _HeaderCard - dark navy/teal/violet gradient (same look in every app theme, a static echo of the RGB theme's own palette), real Hijri date (hijri package) at the row's start, 'مرحبا بك' centered, real localized Gregorian date (DateFormat.yMMMd) at the end - start/end not literal left/right so it mirrors correctly in RTL and LTR. Welcome text stays honestly generic since P3-5 (login, answered: optional) isn't actually built yet - no fake username invented. +6 keys x5 locales. analyze clean, test 15/15. Installed a fresh debug build on the emulator and confirmed live: card renders exactly as designed with today's real dates (٢١ ربيع الأول ١٤٤٨ هـ / ٣ سبتمبر ٢٠٢٦). PHASE3.md updated. Still open for P3-4: per-card Islamic pattern backgrounds, animated prayer-card countdown clock, ornamental hadith-card framing, dedicated continue-reading card.
