...
<!-- WIP:START -->
**2026-09-06 04:04 — IN PROGRESS — resume here**

P3-50: two splash changes the owner asked for. (1) Moved the permission prompts to AFTER the splash - they used to fire from main() (via AdhanAlarmService.initialize) and pop over the splash video on first launch. initialize() now only sets up the plugin/channel and never prompts; a new requestStartupPermissions() is called from the splash's _proceed once it hands off, requesting LOCATION first (the owner's ask) then notifications + exact alarm. Live-verified: the location dialog now appears over onboarding after the splash, not during it. (2) Removed the 'first splash' - deleted the girih-lattice/name/tagline branded intermediate screen (splash_lattice.dart) so the flow is just icon -> video: while the video decodes (or on any device that can't decode it) only the app mark shows on the dark ground, a seamless continuation of the native launch icon. flutter analyze/test clean (21/21); live-verified on a fresh install that the splash shows only the icon and no permission dialog interrupts it.
