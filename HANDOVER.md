...
<!-- WIP:START -->
**2026-09-17 21:40 — IN PROGRESS — resume here**

the red inline countdown, the mushaf rights answered, and the first ever in-app download on the emulator. AVAST IS OFF, and that changed what could be tested: the handshake to the bucket now returns a Let's Encrypt leaf instead of CN=Avast Web/Mail Shield Root, so trap 13 is lifted. I downloaded al-Muwafaqat from inside the app for the first time - 2.2 MB, the card flipped from Download to Open, the book opened on 3122 pages with the khutbat al-hajah and its three ayahs picked out, and the reader's own footer reads المكتبة الشاملة الموافقات للشاطبي plus «the author's text from this printing; the editor's notes are not included». The filtering we did today is stated to the reader, not hidden. THE NOTIFICATION now draws its countdown where it can be read: 7:07:13 in red, inline, beside «Fajr, 4:47 AM». The platform chronometer stays in the header as a fallback, but a RemoteViews Chronometer inside DecoratedCustomViewStyle is what puts it in the body - and it is a Chronometer, not a rewritten string, so nothing wakes once a second. The header, icon and buttons stay the platform's so the card still looks native on any OEM skin. THE THREE UNLICENSED PRINTINGS are answered and open item 6 is closed. Rendered page 1 and 604 of tajweed_color, kuwait and madinah_night: all six are pure Quranic leaves. What we host contains NO title page, imprint, copyright page or colophon - the exact opposite of the Taj Company scan, whose own last page printed جملہ حقوق محفوظ and which was rejected for it. Their archive.org items carry no licenseurl and no rights field either, checked through the metadata API. They stay NO_LICENCE_STATED and CONTENT-LICENSES records them at their real strength: they rest on the absence of a claim, not on a grant, which is weaker than Qatar's stated CC BY-NC-SA and weaker than hafs_kfqc's CC0, and it says so
