...
<!-- WIP:START -->
**2026-09-05 19:46 — IN PROGRESS — resume here**

checkpoint(wip): P3-44 - gzip compression is now a standing rule for every book text edition on R2, per the owner explicit ask. BookText.fromFile now detects gzip by magic bytes before deciding whether to decompress, real backward compatibility since a file downloaded before this change keeps working exactly as before with no forced re-download. build_book_text.py gained a shared write_book_json helper that gzip-compresses at the one place every build script output funnels through, so every future book is compressed by default; fetch_authors_batch.py updated to use it. Retroactively applied to every book already on R2, all 193 local files compressed and re-uploaded and verified via head_object, real result 70.8 MB down to 16.5 MB, 77 percent smaller. Re-derived approxSizeBytes for every catalog entry from the real compressed file size, found and fixed a real regex miss along the way where dart format wraps a long fileName line onto its own line, which silently left exactly one book still showing its old uncompressed size until the regex was loosened. flutter analyze and test clean, 21 of 21. Live-verified a real round trip on emulator-5554, not just inspected: deleted a downloaded book, confirmed the app falls back to an honest error screen instead of crashing on the missing file, redownloaded it (now fetching the new gzip object, confirmed by the much smaller transfer), and opened it with identical real content rendering correctly.
