...
<!-- WIP:START -->
**2026-09-05 13:46 — IN PROGRESS — resume here**

P3-43 item #11 investigated thoroughly, no reproducible cause found in the current codebase: traced every real recitation-download call site by hand (FullRecitationCard._downloadAll and _SurahAudioTile._download) - both only ever fire from an explicit button press, nothing else calls either. Checked every candidate for a hidden locale coupling (SelectedReciter's persisted key, AyahAudioService, onboardingCompletedProvider) and found none - grep for 'locale' across the downloads feature and AyahAudioService returns zero hits. The one auto-download path that did once exist (essential_content_bootstrap.dart's first-launch auto-fetch) was already deleted in an earlier P3-41-follow-up session, which plausibly already fixed this exact complaint before it was even reported this round. Not marked done since 'no cause found by static search' isn't the same as 'confirmed fixed' - documented as needing a real repro on the current build before closing, rather than guessing at a patch for a bug that can't be found or reproduced.
