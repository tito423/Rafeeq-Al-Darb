...
<!-- WIP:START -->
**2026-09-17 15:23 — IN PROGRESS — resume here**

three of the open items closed, and FOUR things this document itself had got wrong, each caught by looking at the repo instead of recalling it. (1) The Quran translations were never the unexamined gap I called them: assets/data/catalogs/quran_translations.json holds 45 - not 47 - each with its translator named, translation_tab.dart already SHOWS him as the subtitle under every block, and alquran.cloud's terms, read today, permit exactly this - free for any non-commercial purpose, edition identifier kept intact, «attribute the translator by name». The app is free, keeps the identifier and prints the name. Compliant, and compliant before the audit started, which deserves writing down as plainly as a defect would. (2) hadeethenc.com's terms had NOT «never been read» - they were read on 2026-09-10 and the reasoning is in AppConfig.hadeethEncUrl's own doc comment. (3) madinah_gold's ND clause: NC holds (free, no ads), BY holds (credited on the Sources screen), and the pipeline takes the item's JPEGs as they are and only downscales them to 1200px for bandwidth - no crop, no recolour, no recomposition - so under the definition of مصنف مشتق quoted at the top of the file it adds nothing to arrangement or selection. I had written that it «trims the scan margin»; it does that for the cover thumbnails, not the pages. And the uploader's own description says, bolded and underlined, that he does not own the content. (4) editions.json had no per-edition provenance at all and its single top-level license line was STALE - still naming Warsh and Qalun, deleted 2026-09-09. All six printings now carry source, license and license_note read from archive.org's own metadata today, and mushaf_provenance_test.dart fails the build on a printing added without them, on a stale top line naming a deleted edition, and on a restrictive licence left as a bare code with no note saying how the app stays inside it. Qatar CC BY-NC-SA 3.0, madinah_gold CC BY-NC-ND 4.0, the other three state none and are recorded as stating none. analyze clean, 297 tests
