...
<!-- WIP:START -->
**2026-09-17 18:00 — IN PROGRESS — resume here**

checkpoint(wip): THE TREE IS RED ON PURPOSE - talib_ilm_shelf_test.dart fails 3 of 6 because the shelf is still empty, and that failure is the proof the test works. It goes green when the catalogue entries land. Do not delete it if you find it failing; finish the catalogue write. What it exists for was caught by READING books_tab.dart instead of trusting the order I wrote the entries in. He asked for الكتب المتدرجة بتدرج and I duly wrote them in teaching order - but the tab sorts every shelf by sortKey, the Arabic collation handle, so on the device the shelf would have opened on الآجرومية with جامع بيان العلم وفضله, the book a student reads FIRST, sitting in the middle. Right in the file, wrong on the screen: section 1.3 in one sentence. So LibraryBook gains shelfOrder, default 0 meaning sort me alphabetically with everyone else, and books_tab sorts on (shelfOrder, sortKey) - every other shelf is untouched. shelfOrder is exactly the kind of field nothing else can check: every value is a valid int, analyze has no opinion, and a book left at the default 0 does not look broken, it looks like step one. So the test asserts the four stages by name, that none is empty, that جامع بيان العلم وفضله is stage 1 and الموافقات is stage 4, and separately that books_tab.dart still mentions shelfOrder at all - a test mirroring production code is worth nothing once production stops doing it. It also pins that الاعتصام is ABSENT deliberately, so a later session does not complete the set. 16 of 27 books are on the bucket; 11 still building
