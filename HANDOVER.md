...
| **Released** | **v3.32.0** — the rights release, completed. v3.31.0 did the cleaning; v3.32.0 is what makes it reach a device. Both older releases and their tags deleted, so the repo holds one |
| **App version** | `pubspec.yaml` `3.32.0+33`; `AboutScreen.appVersion` `3.32.0` — the two are checked against each other by a test |
| **On the owner's Honor** | still the signed **3.25.0** build. **Nothing from v3.26.0 to v3.32.0 has ever run on his phone** — seven releases of unseen-on-hardware work. Everything below was seen on `emulator-5554` |
...
| **Stale downloads** | fixed. `isBookDownloaded` compares the local file's byte length with the catalogue's `sizeBytes`, so a device holding a pre-filter copy is told to download it again. `verify_catalog_sizes.py` reports **0 mismatches over 248** after two entries that had drifted by 7 and 14 bytes were corrected from the bucket's readback. Proven on the emulator in both directions |
...
<!-- WIP:START -->
**2026-09-17 20:31 — IN PROGRESS — resume here**

the 27 books are all in and the library is CLEAN on every measurement. 213 entries, all four last arrivals catalogued, and the طالب العلم shelf complete so its test is green for the first time - 342 tests, no reds. Three of the last four carried apparatus and were filtered: muqaddimat_ibn_al_salah keeps 99.9 percent, al_risalah_lil_shafii 92 with the six empty pages at the FRONT which is the editor's own introduction, and al_muwafaqat 89 percent with 89 empty pages across 3211 and the longest gap 7. That last one was a judgement call and the number that decided it is the shape of the gap, not its count: tuhfat_at_talib was removed this afternoon for a 32-page hole in 486 pages, which is a scatter of fragments; seven pages in three thousand is a book. Final state, all measured after the fact rather than intended: apparatus audit 0 and 0 with the WIDENED detector, 0 unreachable, 213 of 213 catalogue sizes matching the bucket exactly, 43 books flagged editorNotesRemoved. FRONT-END AUDIT, and its main finding is about my own tooling. Three chevron_left hits were false positives - all prev/next arrows where auto-mirroring is correct, two carrying a comment saying so; trap 7 is about DISCLOSURE chevrons. Two orphan screens were false positives, private classes used inside their own file. And the 341 unused locale keys was the dangerous one: classifying instead of counting cut it to 178, and then hajj.step_ turned out to be built as hajj.step_dollar-brace-step.key - an UNDERSCORE before the variable, which my pattern only matched after a dot - so all 39 hajj keys are live. Twice my own detector cried wolf, so NOT ONE KEY WAS DELETED on its evidence; a key removed while something builds it renders as the raw key on a religious screen. The real find was a STALE TODO in app_config warning that mushaf SVGs come from GitHub raw and need mirroring to a bucket. They are already on R2 - verified in this same audit, 206 image/svg+xml for pages 1, 302 and 604 - so the note now records that the migration is done, because a stale TODO would have cost the next session a migration that already happened
