...
<!-- WIP:START -->
**2026-09-08 17:53 — IN PROGRESS — resume here**

Fix the root cause of the download problems: two services were fighting over a single-subscription stream. background_downloader exposes FileDownloader().updates from a plain StreamController, not a broadcast one, so it accepts exactly one listener. Two services both called listen on it - DownloadManager for the hadith DB, books and adhan clips, and AyahAudioService for every ayah of a recitation - so whichever attached second got Bad state: Stream has already been listened to and then received nothing for the rest of the session. That is not cosmetic. A service that never sees a status update never settles its tasks: recitation progress freezes mid-surah, the multi-host fallback that retries a failed ayah on the next CDN never fires so that ayah is lost for good, download-whole-reciter never advances past its first surah, and on the other side the hadith zip is never unpacked or registered even though the bytes arrived. It also depended on which service happened to wire first, which is why it presented as intermittent. DownloadEngine now owns the one subscription and re-broadcasts through its own broadcast controller; both services listen there. Reproduced and verified on the emulator: before the fix al-Baqarah sat frozen at 67/286 in the UI while logcat showed files 142-146 genuinely downloading; after it, a clean run ticks 18 to 32 to 53 live. Separately, the repair button only ever called DownloadManager.resumeAll(), which knows nothing about recitations or mushaf pages - so the two downloads most likely to be interrupted, a 286-file surah and a 604-page edition, were exactly the two it could never touch. Added AyahAudioService.repairPartialDownloads and MushafPageService.repairPartialEditions, both of which resume only genuinely partial work and leave untouched and completed items alone, and rewired the button to run all three and report the real number restarted instead of always claiming success. Verified by killing the app mid-download and tapping repair: the surah resumed from 119 to 143 with the button showing its own progress. Also confirmed for the owner that the hadith category is real and downloadable (hadith.zip, 200, 16.8 MB) and is its own row in the overview rather than living under recitations, so it should not be removed. Wired in the owner's own prayer-steps artwork exactly as supplied and removed the stand-in SVG. flutter analyze clean, flutter test 25/25.
