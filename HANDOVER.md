...
<!-- WIP:START -->
**2026-09-15 21:15 — IN PROGRESS — resume here**

fix: the guided tour flickered on every stop - owner reported it, screenrecord measured it. At 30fps on his Honor the mean frame brightness dropped hard and ramped back TWICE per stop: the bubble faded in, snapped to nothing and faded in again. Two causes, and the first guess was wrong - logging _enter on the device proved the tour logic was innocent (one enter, one measurement per stop), so the fix is not about re-entry. (1) The bubble now changes stop only when the new target has been measured (_shown), instead of rebuilding with the new chapter while the old spotlight was still up. (2) The entrance rides _move, the controller reset exactly once per stop, instead of a keyed TweenAnimationBuilder - which replays from zero whenever its element is rebuilt, and every stop calls onGoToTab, which rebuilds the shell the overlay sits in. (3) The card never reaches zero opacity, so swapping one stop's content for the next cannot read as a blink. Measured over 20s of stepping: 5 anomalous frames (up to 21 grey levels deep) -> 1 (5.8 deep). analyze clean, 221 pass. Tour also walked end to end on the phone: 31/31 reached, 5 stops skipped because their features are switched off.
