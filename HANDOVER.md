...
<!-- WIP:START -->
**2026-09-13 14:25 — IN PROGRESS — resume here**

the owner's Web OAuth client ID is in, stored in AppConfig where the sync work will need it. It is deliberately committed: a Web client ID is the public half - the same value that ships inside every Android app's google-services.json - and it is what google_sign_in needs as serverClientId on Android to return an idToken at all. Without the idToken the sync service has no signed statement of who is calling, and handing back a person's khatma and counters on an unverified claim is not something to build. The client SECRET was explicitly not requested and not taken. The doc comment records why the scopes stay email/profile: drive.appdata is a sensitive scope that re-prompts roughly every seven days on an unverified app, which cannot satisfy «يبقى دايما في مزامنة». NOT yet confirmed and flagged as such in NEXT_PROMPT: whether the two ANDROID clients (debug and release fingerprints) were actually created - he sent only the Web id, and without the Android pair the sign-in cannot happen at all. Still waiting on the Cloudflare Workers token, which he puts in scripts/.env himself; it must never reach the chat or a commit. No sync code written yet - the owner is at 90 on the weekly quota and this is a 60-70 job that is worthless half-built.
