...
<!-- WIP:START -->
**2026-09-05 08:55 — IN PROGRESS — resume here**

P3-43 items #4/#5 done + live-verified: deleted the old surah-name scroll strip and the ‹ › page-arrow buttons entirely (both repeat complaints from P3-41/this round), replaced with one new _FastPageScrollBar - a draggable gold thumb that jumps the mushaf immediately (no animation) to wherever it's dragged/tapped, tracking the live position while dragging. Page number itself isn't lost since P3-43 #7's persistent overlay (this same session) already shows it always. Direction kept as a plain left-to-right value matching every other slider in the app (auto-scroll speed, font size) - a first attempt inverted the mapping on an unverified assumption about the old arrows' RTL behavior, caught before committing by jumping to specific pages (100, then near the end) and watching the thumb land backwards from intended, reverted to the plain unsurprising mapping rather than ship a guess. flutter analyze/test clean (15/15). Live-verified on emulator-5554: jumped to pages 1/300/590 via the toolbar's Jump-to dialog and confirmed the thumb tracks proportionally and correctly (left=low page, right=high page after the revert); confirmed the actual touch-drag gesture itself works end-to-end (dragged the thumb, page changed and thumb followed) once tested away from the screen edges (an early drag attempt near the true screen edge triggered Android's own edge-swipe-back gesture instead, unrelated to the widget - a testing artifact, not an app bug, caught and worked around).
