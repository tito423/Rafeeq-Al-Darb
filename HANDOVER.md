...
<!-- WIP:START -->
**2026-09-17 20:02 — IN PROGRESS — resume here**

the About card credits Tamer Abdelhakim. One occurrence in the whole repo, in about_screen.dart, changed and seen on the device under «Developed by» with the gold shimmer intact. No locale key: a person's own name is written the way he writes it in every language, which is properName's rule, so it stays a literal and ui_strings_translated_test has no quarrel with it because it is Latin script. While the screen was open I checked the one number on it that today's work could have falsified - it says «12 adhan recordings», and that is read from adhans?.length off the live catalogue rather than typed, so restoring azan13 this morning is exactly why it reads 12 again. The page's own blurb promises «none of them is typed by hand» and that promise still holds
