...
<!-- WIP:START -->
**2026-09-13 01:44 — IN PROGRESS — resume here**

v3.23.0 is PUBLISHED and verified - the owner said the quota is at 88 and the release must go out before it ends, so it went out. gh release view reports tag v3.23.0 on master with one asset (rafeeq-aldarb-3.23.0.apk, 340,341,048 bytes) and the tag's SHA equals HEAD 18be905. The APK is the SIGNED one: scripts/sign_release.py reported 'rotated - new key from Android 9 up, old key kept below it', so it installs over 3.22.0 as an update and the hundreds of MB of downloaded mushaf pages survive. v3.22.0 and its tag were deleted first, as the owner wants one release at a time. Shipped in it: the tajweed course (23 lessons read verbatim from تيسير أحكام التجويد by explicit paragraph ranges, each rule heard in a real ayah in a mujawwad recitation), «في مثل هذا اليوم» (366 days per language, ZERO empty, 4388 ar / 4392 en events, opened from both the Hijri and the Gregorian date), focus mode with three destinations, the smart prayer notification, the preferred name, the spotlight tutorial, the mushaf-mode button, the app-wide flicker, six stages of reorganisation and the zip-slip fix. NEXT_PROMPT.md rewritten as one coherent prompt: the ayah-by-ayah downloader rebuilt on everyayah with Range resume, Google sign-in with the exact OAuth console steps the owner asked for, the three Shamela batches that are built and uploaded but NOT in the catalogue and still carry ugly transliterated slugs, and - named as not proven, not as done - the notification countdown jump, which could never be reproduced on the emulator.
