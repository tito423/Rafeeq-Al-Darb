...
<!-- WIP:START -->
**2026-09-03 13:55 — IN PROGRESS — resume here**

P2-9 DONE (HOSTING.md written + client wiring re-confirmed) + P2-11 DONE (Quran Khatma tracker card): owner asked to verify the Cloudflare(static)+Firebase-Firestore-Spark(dynamic) hosting split he recalled - confirmed correct, wrote HOSTING.md formalizing it (real content inventory ~38MB migration candidate, exact R2 provisioning steps incl. rotating the exposed token first, direct scale/cost answer: R2 free egress + Firestore's daily-quota-then-stops-serving means nothing in this plan can bill without an explicit later Blaze opt-in). Confirmed by reading source: AppConfig already has both --dart-define override seams, zero Firebase packages in pubspec.yaml (project exists+gitignored, correctly unused until a real feature needs it). P2-11: new khatma_store.dart (3 modes, dynamic catch-up due-amount calc, local persistence) + khatma_reminder_service.dart (mirrors AzkarReminderService) + quran_jump_provider.dart (cross-tab IndexedStack jump seam) + khatma_card.dart (Home top) + khatma_screen.dart (full manager: create sheet, read-today, reminder, delete, history). +26 keys x5. Emulator-verified full flow: created a 4-pages/day khatma, read-today updated the card to 1%/4 pages, switching to Quran tab opened the reader on exactly page 5/604. Owner told to go P2-11->P2-12->P2-9(done)->P2-10->P2-13->P2-7(last). analyze clean, test 13/13.
