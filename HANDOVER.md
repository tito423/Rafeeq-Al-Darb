...
<!-- WIP:START -->
**2026-09-05 13:33 — IN PROGRESS — resume here**

P3-43 item #9 done + live-verified end to end: real quarter-hizb (rub el hizb) khatma pacing. No hizb/quarter column exists in the bundled DB (confirmed by schema check, not assumed) - real per-ayah rub_el_hizb_number data fetched from api.quran.com (this project's own already-trusted tafsir/translation source) by iterating all 30 juz, derived all 240 real quarter-hizb start pages the same first-occurrence rule already used for juz/surah boundaries, verified (6236 ayahs, exactly 240 distinct rubs 1-240, non-decreasing, rub 1 starts page 1) before shipping as assets/data/mushaf/rub_el_hizb_pages.json. New KhatmaMode.dailyQuarters is its own enum value, not a reinterpretation of dailyJuz's existing unit, so no already-created khatma's pacing can be silently misread. Wizard's unit SegmentedButton gained a third Quarters option; picking it swaps the stepper for a bounded 1-7 dropdown with the reference's exact labels (Quarter/Two quarters/3 quarters/One hizb/5-7 quarters). +8 keys x 6 locales. Added a permanent khatma_quarters_test.dart (6 cases) verifying the real page-span math directly, since this session's own manual UI tap-throughs kept missing button coordinates unreliably on this emulator - a repeatable automated test is more durable evidence than one successful manual click-through anyway. flutter analyze/test clean (21/21). Live-verified anyway once tap coordinates were finally sorted out: created a real khatma (Two quarters/day, linked duration auto-computed to 120 days = 240 total real quarters / 2), Read Today correctly jumped to page 7 matching the real fetched rub boundaries exactly, Undo snackbar appeared - a complete real cycle, not a mockup.
