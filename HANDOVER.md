...
<!-- WIP:START -->
**2026-09-05 18:31 — IN PROGRESS — resume here**

checkpoint(wip): P3-44 library mega-expansion - 182 more real titles added across the same 5 established authors (Ibn Abi al-Dunya 57, al-Hakim al-Tirmidhi 3, Ibn Taymiyyah 60, Ibn al-Qayyim 29, Ibn al-Jawzi 33), per the owner explicit ask to expand all the authors. Every id read directly off each author real shamela.ws page; known multi-volume works (zad al-maad, madarij al-salikin, ilam al-muwaqqiin, sifat al-safwa, al-muntazam, and many more) deliberately excluded as too large for this per-page-walk pipeline rather than risking a silent merge across volumes. Built and verified individually via build_book_text.py, 182 out of 182 succeeded with zero failures and zero empty pages across every single book. Categorized into the app real 7 catalog categories via keyword rules read from each real title, spot checked and fixed 5 false positives by hand before trusting the rest. Uploaded all 182 to R2, verified via head_object and a live curl check against the real public URL for 7 titles across all 5 authors. Found and fixed a real data quality bug caught only by actually looking at the live app: 3 of 182 books displayed their internal slug instead of a real title because a handful of Shamela editions format their meta card with a space before the colon, which the extraction regex did not handle - fixed the regex itself in build_book_text.py so it helps every future book too, re-fetched the real metadata for the affected 3, re-uploaded the corrected files, and re-verified live that every title now renders correctly. flutter analyze and test clean, 21 of 21. Live-verified end to end on emulator-5554: the Library tab lists all 182 books with correct real titles, authors, categories, and sizes, downloaded and opened one in the real text reader and confirmed real content, source label, and page navigation all work correctly.
