...
<!-- WIP:START -->
**2026-09-05 17:35 — IN PROGRESS — resume here**

checkpoint(wip): P3-44 - fixed the shared Azkar/Tasbeeh settings sheet showing the same morning/evening adhkar reminder section on both tabs. Real-device feedback pointed out that reminder concept only makes sense on the Azkar tab, not on Tasbeeh's free-form counter. AzkarSettingsButton now takes a showReminders flag (true on Azkar, false on Tasbeeh) so the one section that does not apply everywhere is conditional instead of always shown; the haptics toggle stays on both since it genuinely drives tap feedback for both counters. flutter analyze and test clean, 21 of 21.
