...
<!-- WIP:START -->
**2026-09-17 14:20 — IN PROGRESS — resume here**

CONTENT-LICENSES.md updated to the state after the fix, and the state is MEASURED not claimed: audit_editor_apparatus.py was run again over all 248 books and the row that matters - a modern editor's apparatus inside a file we redistribute - returns ZERO, where it was 48 this morning. The doc now carries a before/after column, what was done to the 48 and why the WHERE of an empty page run is the number that decides whether a filtered book is still a book, the one removal with its measurements, and two things left open that I am not pretending are closed: a filtered book's source label still names the muhaqqiq, which is true about where the text came from but could read as «this is his edition», and wants one flag on TextEdition plus one translated line in seven locales rather than 47 hand-edited labels; and book_meta carries no version, so a device that already downloaded a book keeps the old text - hadith.db has AppConfig.hadithDbVersion for exactly this and books have no equivalent
