...
<!-- WIP:START -->
**2026-09-17 18:45 — IN PROGRESS — resume here**

checkpoint(wip): the Quran toolbar, rebuilt and seen on the device. He said شكلهم بدائي اوي and he was right about more than the look. ELEVEN captioned tiles wrapped onto two rows above the text, and three faults: three of them (السور، الجزء، الانتقال) all opened showJumpSheet, which has been ONE sheet with three tabs since he asked for it - two were duplicates of the third. Nothing showed its current value: every tile was an icon and a verb, so the only way to learn which layout you were in was to press something and watch the page change. And the bar spent about a quarter of the phone height, permanently, on the tab whose whole job is to show the Quran. Now four actions stay - انتقال، بحث، استماع، عرض - and every SETTING moved into a grouped sheet: three layout cards with the active one filled gold, a font row showing the size as a live Arabic sample AND a percentage so the 1.8 ceiling is visible instead of the plus button just going quiet, the page-colour link, full-screen and auto-scroll as switches, and the mushaf and mode. Verified on emulator-5554 on a debug build: one row, and the text now starts about 114 of 1200 pixels higher. All eight tutorial anchors still resolve - the five whose buttons moved are nested on عرض, which is where those settings now are, so the tour spotlights the button that opens them rather than pointing at nothing. ui_strings_translated_test caught the Arabic font-preview literal immediately; it is content, not UI text - the control sizes Quranic script so the sample has to BE that script - and it is on the allow-list with that reason. code_layout_test then caught the screen growing by one line past its grandfathered 1047, twice: once for the new parameter and once for my own comment. Both paid for out of the same file rather than by raising the ratchet. analyze clean, 337 tests, one red and waiting on al_muwafaqat to finish building
