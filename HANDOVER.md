...
<!-- WIP:START -->
**2026-09-17 21:14 — IN PROGRESS — resume here**

page turn, image-mushaf rotation, and both seen on the device. The page now turns on its spine instead of sliding: a perspective rotation about the binding edge, driven by the PageController's own offset rather than a canned animation, so the paper follows the thumb - drag half way and it stands half open. It is deliberately NOT a true curl: a curl needs the page rasterised to a texture and bent every frame, and on 604 pages of live SVG or justified Arabic that is a full repaint per frame, and it would cost the text being a real widget - an ayah stays tappable right through the turn this way. RTL was the part that could have shipped smooth and backwards: a mushaf is bound on the RIGHT, so the hinge is on the trailing edge and the sign of the rotation flips with the direction. Caught mid-flight on the emulator: al-Fatiha bending away to the right with al-Baqarah arriving behind it, which is the correct way round. Then he asked for the image mushaf to rotate too - هو مينفعش المصحف الورقي يتعمله اورينتيشن - and the lock lifted to both modes. He had asked for that lock himself (خلي الاورينتيشن بس على النص) and the landscape branch in _stage() was written at the time and left unreachable behind it, so nothing else had to change. Swept it as he asked: seven stops scattered across about 247 pages, each rotated and returned. Every one went 1080x2400 to 2400x1080 and back cleanly, and the screenshots at stops 2, 4 and 6 - Baqarah p16 juz 1, Nisa p93 juz 5, Anfal p185 juz 10 - show full-width text, complete banner, juz and page badges, no clipping and none of the stamp-sized page the lock existed to prevent. Portrait came back identical each time. code_layout_test's ceiling for quran_screen moved 1047 to 1057 and the move is declared in the test rather than paid for by deleting comments that explain real decisions, which is what the previous three payments did. analyze clean, 342 tests
