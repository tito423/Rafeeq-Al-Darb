...
<!-- WIP:START -->
**2026-09-08 14:57 — IN PROGRESS — resume here**

Fix per-edition page counts, wrong running headers, and unverified muezzin attributions. Three real defects, two of them introduced or exposed by adding printings that do not paginate like Hafs. (1) Page count was hardcoded to 604 everywhere, but Shamarly is 521 pages and the Indo-Pak printing 564, so the reader offered pages that do not exist (blank 404s past the real end) and the offline download walked past the end and would abort on its three-consecutive-errors guard before finishing. The reader now takes the open edition's own page count and pulls the reader back to the last real page when switching to a shorter printing; cachedPages and prefetchEdition take the edition's total from their callers. Verified on the emulator: asking to go to page 604 while reading Shamarly lands on page 521, its real last page with al-Ikhlas, al-Falaq and an-Nas, instead of a blank. (2) That same verification exposed the running header naming the wrong surah - it read Adh-Dhariyat, juz 26 over Shamarly's last page, because the page-to-surah and page-to-juz mapping in the sciences DB is Hafs 604-page pagination. Added a hafs_pagination flag to editions.json, true only for the printings that genuinely use the Madinah layout (hafs_kfqc, tajweed_color, madinah_gold) and false for Shamarly, Indo-Pak, Warsh and Qalun, whose 604 pages still do not line up verse for verse. The header is now omitted on those rather than asserting a surah the page does not hold. (3) All ten bundled adhans were attributed to named muezzins with no source stating who recites which file - they came from islamcan.com's generic numbered azan1..10.mp3, every entry in adhans.json carried its own admission that the name needed confirming, and islamcan's adhan pages now 404 so it cannot be checked there even in principle. Replaced all ten from archive.org's adhan-mp3-collection, which carries the muezzin in each filename and had a real recording for every name on the owner's list, including the three that needed Haram spellings (ali-ibn-ahmad-mala, assem-bukhrare, muhammad-khaleel-raml). New scripts/fetch_named_adhans.py downloads them, checks each is a real MP3 of plausible length, overwrites the bundled asset and rewrites the catalog, so no code change was needed and zero unverified attributions remain. One deliberate correction: the request said Mustafa al-Banna but the only al-Banna adhan in the collection is Mahmoud Ali al-Banna, so it is catalogued under his real name rather than relabelled to match. Also retagged v2.1.20, v2.1.21, v3.0.0 and v3.0.1 locally onto their real commits, each annotated with the timestamp and title evidence for the mapping - they had all pointed at the repo's initial commit because the releases were cut with --target main while all work lives on master. Pushing those tags needs the owner's approval. flutter analyze clean, flutter test 25/25.
