...
<!-- WIP:START -->
**2026-09-06 13:54 — IN PROGRESS — resume here**

P3-52 Full-Screen Azan Player (live-overlay architecture). New adhan_full_screen_screen.dart: Stack UI = looping silent video background (BoxFit.cover) or animated gradient fallback, dark scrim for legibility, content layer with prayer name + live HH:mm:ss clock (top), current adhan phrase in AmiriQuran with AnimatedSwitcher fade+scale (middle), Mute + Dismiss buttons (bottom). Audio via just_audio owning playback; new azan_subtitle.dart builds a subtitle timeline distributed across the recording's real duration (weighted by phrase length x repeat) so karaoke text tracks the actual audio regardless of muezzin pacing; positionStream drives the active phrase, playerState completed auto-dismisses. full mode notification is now a SILENT fullScreenIntent trigger (channel radh_full_silent, playSound false, Importance/Priority max, ongoing, autoCancel false) so the screen owns the audio and avoids double-play; audio-only mode keeps native channel sound. audioFilePath plumbed through buildAdhanPayload/AdhanPayload/adhan_scheduler/adhan_navigation. Added prayer.azan_of key to all 6 locales (parity enforced). Clean dispose of video+audio controllers, timers, and stream subs. flutter analyze clean, flutter test 21/21. On-device items for owner real device: fullScreenIntent lock-screen launch, adhan audio + subtitle sync (emulator does not honor fullScreenIntent over a foregrounded app nor play audio audibly).
